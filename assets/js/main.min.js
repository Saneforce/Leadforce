...
function init_rel_tasks_table(e, t, a) {
	...
	if (0 !== i.length) {
        ...
        if(i.attr("data-new-bycall") && i.attr("data-new-bycall") == 'bycall') {
            var l = admin_url + "tasks/init_relation_tasks/" + e + "/" + t;
            "project" == i.attr("data-new-rel-type") && (l += "?bulk_actions=true&call=bycall"), initDataTable(i, l, o, o, s, [])
        } else {
            var l = admin_url + "tasks/init_relation_tasks/" + e + "/" + t;
            "project" == i.attr("data-new-rel-type") && (l += "?bulk_actions=true"), initDataTable(i, l, o, o, s, [])
        }
...
function dt_custom_view(e, t, a, i) {
	...
	var cur_val = $('input[name="' + n + '"]').val();
	var cur_val1 = '';
	var req_task 		= $('input[name="my_tasks"]').val();
	var req_task_type 	= $('input[name="req_task_type"]').val();
	var req_task_assign = $('input[name="req_task_assign"]').val();
	if(cur_val == '' || cur_val == 'task_status_1' || cur_val == 'task_status_2' || cur_val == 'task_status_3' || cur_val == 'task_status_4' || cur_val == 'task_status_5' || req_task != '' || req_task_type!='' || req_task_assign!=''){
		var all_period = $('input[name="all_period"]').val();
		if(all_period != ''){
			cur_val1 = all_period;
		}
		var today_tasks = $('input[name="today_tasks"]').val();
		if(today_tasks != ''){
			cur_val1 = today_tasks;
		}
		var tomorrow_tasks = $('input[name="tomorrow_tasks"]').val();
		if(tomorrow_tasks != ''){
			cur_val1 = tomorrow_tasks;
		}
		var yesterday_tasks = $('input[name="yesterday_tasks"]').val();
		if(yesterday_tasks != ''){
			cur_val1 = yesterday_tasks;
		}
		var thisweek_tasks = $('input[name="thisweek_tasks"]').val();
		if(thisweek_tasks != ''){
			cur_val1 = thisweek_tasks;
		}
		var lastweek_tasks = $('input[name="lastweek_tasks"]').val();
		if(lastweek_tasks != ''){
			cur_val1 = lastweek_tasks;
		}
		var nextweek_tasks = $('input[name="nextweek_tasks"]').val();
		if(nextweek_tasks != ''){
			cur_val1 = nextweek_tasks;
		}
		var thismonth_tasks = $('input[name="thismonth_tasks"]').val();
		if(thismonth_tasks != ''){
			cur_val1 = thismonth_tasks;
		}
		var lastmonth_tasks = $('input[name="lastmonth_tasks"]').val();
		if(lastmonth_tasks != ''){
			cur_val1 = lastmonth_tasks;
		}
		var nextmonth_tasks = $('input[name="nextmonth_tasks"]').val();
		if(nextmonth_tasks != ''){
			cur_val1 = nextmonth_tasks;
		}
		var custom_tasks = $('input[name="custom_tasks"]').val();
		if(custom_tasks != ''){
			cur_val1 = custom_tasks;
		}
		var upcoming_tasks = $('input[name="upcoming_tasks"]').val();
		if(upcoming_tasks != ''){
			cur_val1 = upcoming_tasks;
		}
	}
	else{
		if(cur_val == 'all_period' || cur_val == 'today_tasks' || cur_val == 'tomorrow_tasks' || cur_val == 'yesterday_tasks' || cur_val == 'thisweek_tasks' || cur_val == 'lastweek_tasks' || cur_val == 'nextweek_tasks' || cur_val == 'thismonth_tasks' || cur_val == 'lastmonth_tasks' || cur_val == 'nextmonth_tasks' || cur_val == 'custom_tasks' || cur_val == 'upcoming_tasks' ){
			cur_val1 = cur_val;
			cur_val = '';
		}
		
	}
	var req_task_type = req_task_assign = '';
	if( a.includes("task_tasktype_")){
		var cur_task_type = a.split('task_tasktype_');
		var req_type = cur_task_type[1];
		var task_type = $('input[name="req_task_type"]').val();
		var req_task_type = '';
		if(e!=''){
			if(task_type!=''){
				req_task_type = task_type + ',' + e;
			}
			else{
				req_task_type = e;
			}
			...
			var cur_task_type = $('input[name="req_task_type"]').val();
			var arrs = cur_task_type.split(',');
			if(arrs.length>0){
				for( var j = 0; j<arrs.length;j++){
					if(req_type != arrs[j]){
						if(j == 0){
							req_task_type = arrs[j];
						}
						else{
							req_task_type = req_task_type + ','+arrs[j];
						}
					}
				}		
			}
		}
		$('input[name="req_task_type"]').val(req_task_type);
	}
	if(a.includes("task_assigned_")){
		var cur_task_assign = a.split('task_assigned_');
		var req_assign = cur_task_assign[1];
		var task_assign = $('input[name="req_task_assign"]').val();
		if(e!=''){
			if(task_assign!=''){
				req_task_assign = task_assign + ',' + e;
			}
			else{
				req_task_assign = e;
			}
			...
			var cur_task_assign = $('input[name="req_task_assign"]').val();
			var arrs = cur_task_assign.split(',');
			if(arrs.length>0){
				for( var j = 0; j<arrs.length;j++){
					if(req_assign != arrs[j]){
						if(j == 0){
							req_task_assign = arrs[j];
						}
						else{
							req_task_assign = req_task_assign + ','+arrs[j];
						}
					}
				}		
			}
		}
		$('input[name="req_task_assign"]').val(req_task_assign);
	}
	var task_type	= $('input[name="req_task_type"]').val();
	var task_project= $('input[name="task_project"]').val();
	var task_assign = $('input[name="req_task_assign"]').val();
	var period_from = $('input[name="period-from"]').val();
	var period_to	= $('input[name="period-to"]').val();
	var my_tasks	= $('input[name="my_tasks"]').val();
	 var url		= admin_url+'tasks/table_summary';
	$.ajax({
		type: "POST",
		url: url,
		data: {task_type:task_type,task_assign:task_assign,cur_val:cur_val,cur_val1:cur_val1,my_tasks:my_tasks,period_from:period_from,period_to:period_to,task_project:task_project},
		success: function(response){
			var myArr = JSON.parse(response);
			$('.Overdue1').html(myArr['overdue']);
			$('.Today1').html(myArr['today']);
			$('.Upcoming1').html(myArr['upcoming']);
			$('.Completed1').html(myArr['completed']);
			
			var assign_text = $('#assign_me_text').val();
			$('#me_Overdue').html(assign_text+': '+myArr['overdue_me']);
			$('#me_Today').html(assign_text+': '+myArr['today_me']);
			$('#me_Upcoming').html(assign_text+': '+myArr['upcoming_me']);
			$('#me_Completed').html(assign_text+': '+myArr['completed_me']);
		}
	}); 
}
...
function init_roles_permissions(e, t) {
	if (e = void 0 === e ? $('select[name="role"]').val() : e, !($('.member > input[name="isedit"]').length > 0 && void 0 !== e && void 0 === t) && !0 !== $('input[name="administrator"]').prop("checked") && "" !== e) {
		...
		requestGetJSON("staff/role_changed1/" + e).done(function (e) {
...
function init_lead(e, t) {
	if(e>0){
		window.location.href = admin_url+'leads/lead/'+e
	}
...
function validate_lead_profile_text_input(b, a) {
    return (
        $(".not-valid-" + a).remove(),
        $('[name="' + a + '"]')
            .parent()
            .removeClass("has-error"),
        !!(0 == b.length || b.match(/^\d*[a-z][a-z\d`!@#$%^&*()_+\-=\[\]{};':"\\|,.<>\/?~\s]*$/i)) ||
            ($('[name="' + a + '"]')
                .parent()
                .addClass("has-error"),
            $('[name="' + a + '"]')
                .parent()
                .append('<p class="text-danger not-valid-' + a + '" style="">Invalid data</p>'),
            !1)
    );
}
function validate_lead_profile_no_space(b, a) {
    return (
        $(".not-valid-" + a).remove(),
        $('[name="' + a + '"]')
            .parent()
...
function validate_lead_profile_phonenumber(value,name){
	if(value.length ==0){
		return true;
	}
	if(validate_lead_profile_no_space(value,name) ==false){
		return false;
	}
	if(value.match(/^\d+$/) == false || value.match(/^\d+$/) ==null){
		$('[name="'+name+'"]').parent().parent().parent().addClass('has-error');
		$('[name="'+name+'"]').parent().parent().parent().append('<p class="text-danger not-valid-' + name + '" style="">Invalid data</p>');
		return false;
	}
}

function validate_lead_profile_company (value){
	var name ='company';
	if(validate_lead_profile_text_input(value,name) == false){
		return false;
	}
	if($('[name="'+name+'"]').val().length==0){
		if($('[name="clientphonenumber"]').val()|| $('[name="website"]').val()|| $('[name="address"]').val()|| $('[name="city"]').val()|| $('[name="state"]').val()|| $('[name="zip"]').val()){
			$('[name="'+name+'"]').parent().addClass('has-error');
			$('[name="'+name+'"]').parent().append('<p class="text-danger not-valid-' + name + '" style="">Company name cannot be empty</p>');
			return false;
		}
	}
...
function validate_lead_profile_personname (value){
	var name ='personname';
	if(validate_lead_profile_text_input(value,name) == false){
		return false;
	}
	if($('[name="'+name+'"]').val().length==0){
		if($('[name="title"]').val()|| $('[name="email"]').val()|| $('[name="phonenumber"]').val()){
			$('[name="'+name+'"]').parent().addClass('has-error');
			$('[name="'+name+'"]').parent().append('<p class="text-danger not-valid-' + name + '" style="">Person name cannot be empty</p>');
			return false;
		}
	}
...
function validate_lead_profile_form() {
    var a = !0;
    return (
        $('#lead_form input[name="name"]').each(function (b) {
            !1 == validate_lead_profile_text_input($(this).val(), "name") && (a = !1);
        }),
        $('#lead_form input[name="title"]').each(function (b) {
            !1 == validate_lead_profile_text_input($(this).val(), "title") && (a = !1);
        }),
        $('#lead_form input[name="website"]').each(function (b) {
            !1 == validate_lead_profile_text_input($(this).val(), "website") && (a = !1);
        }),
        $('#lead_form input[name="phonenumber"]').each(function (b) {
            !1 == validate_lead_profile_phonenumber($(this).val(), "phonenumber") && (a = !1);
        }),
        $('#lead_form input[name="company"]').each(function (b) {
            !1 == validate_lead_profile_company($(this).val(), "company") && (a = !1);
        }),
		$('#lead_form input[name="personname"]').each(function (b) {
            !1 == validate_lead_profile_personname($(this).val(), "personname") && (a = !1);
        }),
        $('#lead_form input[name="address"]').each(function (b) {
            !1 == validate_lead_profile_text_input($(this).val(), "address") && (a = !1);
        }),
        $('#lead_form input[name="city"]').each(function (b) {
            !1 == validate_lead_profile_text_input($(this).val(), "city") && (a = !1);
        }),
        $('#lead_form input[name="state"]').each(function (b) {
            !1 == validate_lead_profile_text_input($(this).val(), "state") && (a = !1);
        }),
        $('#lead_form input[name="zip"]').each(function (b) {
            !1 == validate_lead_profile_no_space($(this).val(), "zip") && (a = !1);
        }),
        a
    );
}
...
function lead_profile_form_handler(e) {
	var t = (e = $(e)).serialize();
	if(validate_lead_profile_form() === false){
		return false;
	}
...
function task_mark_as(status, task_id, url) {
    ...
    requestGetJSON(url).done(function(response) {
        ...
        if (response.success === true || response.success == 'true') {
			if($('.tasks-kanban').length === 0){reload_tasks_tables();}else{tasks_kanban();}
            ...
            if ($('#_task_modal').length !=0) {$('#_task_modal').modal('toggle'); alert_float('success', response.message); }
        }
    });
}
/*function task_mark_as(e, t, a) {
	if (e == 5 && app.options.mark_complete_mandatory_to_add_new_activity == 1) {
		localStorage.setItem("task_mark_complete_id", t);
		new_task();
	} else {
		a = void 0 === a ? "tasks/mark_as/" + e + "/" + t : a;
		var i = $("#task-modal").is(":visible");
		a += "?single_task=" + i, $("body").append('<div class="dt-loader"></div>'),
			requestGetJSON(a).done(function (a) {
				$("body").find(".dt-loader").remove(), !0 !== a.success && "true" != a.success || (tasks_page_reload(),
					i && _task_append_html(a.taskHtml), 5 == e && "function" == typeof _maybe_remove_task_from_project_milestone && _maybe_remove_task_from_project_milestone(t),
...
function new_task(e) {
	e = void 0 !== e ? e : admin_url + "tasks/task?rel_type=project_task&rel_id=" + localStorage.getItem("task_mark_complete_id");
	var t = $("#lead-modal");
	...
	i.is(":visible") && i.modal("hide"), requestGet(e).done(function (e) {
		...
		document.getElementById("task_mark_complete_id").value = localStorage.getItem("task_mark_complete_id");
		localStorage.setItem("task_mark_complete_id", '');
	}).fail(function (e) {
		alert_float("danger", e.responseText)
	})
}
...
function new_task_from_relation(e, t, a) {
	void 0 === t && void 0 === a && (a = $(e).data("new-rel-id"), t = $(e).data("new-rel-type")),
...
function edit_task_relation(t, a, d)
{
	requestGet("tasks/task/" + t + "?rel_id=" + d + "&rel_type=" + a).done(function (t)
	{
		$("#_task").html(t), $("#task-modal").modal("hide"), $("body").find("#_task_modal").modal(
		{
			show: !0,
			backdrop: "static"
		})
...
function edit_task_new(e) {
	requestGet("tasks/edit_task/" + e).done(function (e) {
		$("#_task").html(e), $("#task-modal").modal("hide"), $("body").find("#_task_modal").modal({
			show: !0,
			backdrop: "static"
		})
...
function task_form_handler(a) {
    ...
    return $.ajax({
        type: $(a).attr("method"),
        data: c,
        mimeType: $(a).attr("enctype"),
        contentType: !1,
        cache: !1,
        processData: !1,
        url: b
    }).done(function(a) {
		if(JSON.parse(a).success ==false && typeof(JSON.parse(a).message) !== "undefined"){
			alert_float("danger", JSON.parse(a).message);
			return ;
		}
        if ((!0 === (a = JSON.parse(a)).success || "true" == a.success) && alert_float("success", a.message), $("body").hasClass("project")) {
            var c = window.location.href,
            b = [];
            c = c.split("?");
            var d = get_url_param("group"),
            e = get_url_param("exclude_completed");
            d && (b.group = d),
            e && (b.exclude_completed = e),
            b.taskid = a.id,
            window.location.href = buildUrl(c[0], b)
        } else $("#_task_modal").attr("data-task-created", !0),
        $("#_task_modal").modal("hide"),
        reload_tasks_tables(),
        $("body").hasClass("kan-ban-body") && $("body").hasClass("tasks") && tasks_kanban()
    }).fail(function(a) {
        alert_float("danger", JSON.parse(a.responseText))
    }), !1
}
...
function init_task_modal(e, t) {
	var a = "",
		i = $("#lead-modal"),
		n = $("#_task_modal");
	i.is(":visible") ? (a += "?opened_from_lead_id=" + i.find('input[name="leadid"]').val(), i.modal("hide")) : void 0 != n.attr("data-lead-id") && (a += "?opened_from_lead_id=" + n.attr("data-lead-id"))
}
...
function init_ajax_lead_search_by_customer_id(e) {
	init_ajax_search("lead", e = void 0 === e ? "#lead_id.ajax-search" : e, {
		customer_id: function () {
			return $('select[name="clientid"]').val()
		}
	})
	...
	if (n.length) {
		var s = {
			...
			preprocessData: function (e) {
				for (var t = [{value: "",text: s.locale.emptyTitle,}], a = e.length, i = 0; i < a; i++) {
					var n = {
						value: e[i].id,
						text: e[i].name
					};
...
var original_top_search_val, table_leads, table_activity_log, table_estimates, table_invoices, table_tasks, tab_active = get_url_param("tab"),
	...
	postid = 0,
	setup_menu_item = $("#setup-menu-item11");
$("body").on("loaded.bs.select change", "select.ajax-search", function (e) {
	...
	totalUnreadNotifications > 0 && (document.title = doc_initial_title), $(".screen-options-btn").on("click", function () {
	...
	}), add_hotkey("Ctrl+Shift+E", function () {}), add_hotkey("Shift+L", function () {
		init_lead()
	...
	if (i && $("#search_input").val(i).trigger("keyup"), $("body").on("blur", "#timesheet_duration", function () {
		...
		var n = {
			...
			header: {
				...
				center: "title",
				right: "month,agendaWeek,agendaDay,viewFullCalendar,viewSmallCalendar"
			},
			editable: !1,
			...
			dayClick: function (e, t, a) {
				...
				return new_task(), setTimeout(function () {
					if ($.datepicker.formatDate('dd-mm-yy', new Date()) + " 00:00" != s) {
						$("input[name='startdate'].datetimepicker").val(s)
					}
				}, 1200)
			}
		};
		if ($("body").hasClass("calendar") && (n.customButtons.viewSmallCalendar = {
				text: "Collapse",
				click: function () {
					window.location.href = admin_url
				}
	...
	var l = {
		...
		teamleader: "[name='view_teamleaders']",
	...
	if ((table_leads = $("table.table-leads")).length) {
		...
		d.length > 0 && (r.push(d.index()), c.push(d.index())), _table_api = initDataTable(table_leads, admin_url + "leads/table", c, r, l,  [0, 'desc']), _table_api && d.length > 0 && _table_api.on("draw", function () {
	...
	if ($("body").on("change", 'input[name="contacted_today"]', function () {
			...
			e.preventDefault(), setup_menu.addClass("true" == isRTL ? "fadeOutRight" : "fadeOutLeft"), $("div#wrapper").removeClass("wrapperstylechange"), $(".btn-bottom-toolbar").css("width", "100%"), requestGet("misc/set_setup_menu_closed")
		}), $(".open-customizer").on("click", function (e) {
			e.preventDefault(), setup_menu.hasClass("true" == isRTL ? "fadeOutRight" : "fadeOutLeft") && setup_menu.removeClass("true" == isRTL ? "fadeOutRight" : "fadeOutLeft"), setup_menu.addClass("display-block " + ("true" == isRTL ? "fadeInRight" : "fadeInLeft")), $("div#wrapper").addClass("wrapperstylechange"), $(".btn-bottom-toolbar").css("width", "84%"), is_mobile() || requestGet("misc/set_setup_menu_open"), mainWrapperHeightFix()
	...
	if ((table_tasks = $(".table-tasks")).length) {
		...
		$("body").hasClass("tasks-page") && (v += "?bulk_actions=true"), _table_api = initDataTable(table_tasks, v, h, h, f, []), _table_api && $("body").hasClass("dashboard") && _table_api.column(4).visible(!1, !1).column(5).visible(!1, !1).columns.adjust()
	}
	...
	b.length > 0 && (b = b.sortable({
		...
		$("#estimate_send_to_client_modal").modal("hide"), $("#proposal_send_to_customer").modal("hide"), $("#invoice_send_to_customer").modal("hide")
		...
		var e = $(this).val(),
			...
			a = t.html("").clone(),
			i = $(".projects-wrapper");
		if (t.selectpicker("destroy").remove(), t = a, $("#project_ajax_search_wrapper").append(a), init_ajax_project_search_by_customer_id(), clear_billing_and_shipping_details(), !e) return $("#merge").empty(), $("#expenses_to_bill").empty(), $("#invoice_top_info").addClass("show"), i.addClass("show"), !1;
...
$("body").on('click', '.transaction-submit', function() {
	...
	if (form.valid()) {
		if (that.hasClass('save-as-draft')) {
		...
		} else if (that.hasClass('save-and-send') && that.hasClass('proposal-form-submit')) {
		...
		} else if (that.hasClass('save-and-send') && that.hasClass('invoice-form-submit')) {
			var cid;
			var relid;
			cid = $('#clientid').val();
			relid = $('#rel_id').val();
			if(cid) {
				if(cid.length == 0 && relid.length == 0) {
					alert('Please select Deal or Organization.');
					return false;
				}
			} else {
				if(relid.length == 0) {
			...
			$('#invoice_send_to_customer').modal({backdrop: 'static', keyboard: false});
			var reltype = $('#rel_type').val();
			$.ajax({
				url: admin_url + 'proposals/get_deals_primary_email_id',
				type: 'post',
				data: {
					cid: cid,
					relid: relid,
...
function projects_kanban_update(ui, object) {
	if (object === ui.item.parent()[0]) {
		...
		var order = [];
		var status = $(ui.item).parents('.project-status').find('li');
		var i = 1;
		...
		data.order = order;
		setTimeout(function () {
			$.post(admin_url + 'projects/update_project_status', data).done(function (response) {
...
function init_projects_status_sortable() {
	if (app.user_is_admin && app.user_is_admin > 0) {
		$("#kan-ban").sortable({
			helper: 'clone',
			item: '.kan-ban-col',
			update: function (event, ui) {
				...
				var data = {};
				data.order = order;
				$.post(admin_url + 'projects/update_status_order', data);
			}
		});
	}
}
...
function project_kanban(search) {
	init_kanban('projects/kanban', projects_kanban_update, '.projects-status', 315, 360, init_projects_status_sortable);
}
// Init the leads kanban
function project_kanban_forecast(search) {
	init_kanban('projects/forecast', projects_kanban_update, '.projects-status', 315, 360, init_projects_status_sortable);
}
// page reload
...
// page reload
function tasks_page_reload() {
	reload_tasks_tables();
	// gettasks_summary_data();
}

function edit_notes(id) {
	requestGet('projects/notesbyid/' + id).done(function (response) {
		json = JSON.parse(response);
		console.log(json.content);
		$('#noteid').val(json.id);
		$('#editcontent').val(json.content);
		$("body").find('#edit_note').modal({
			show: true,
			backdrop: 'static'
		});
	});
...
if ($('#setup-menu-wrapper').hasClass('display-block')) {
	$("#wrapper").addClass("wrapperstylechange");
	$(".btn-bottom-toolbar").css("width", "84%");
}
