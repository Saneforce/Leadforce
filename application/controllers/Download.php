...
class Download extends App_Controller
{
    ...
    public function file($folder_indicator, $attachmentid = '')
    {
        ...
        } elseif ($folder_indicator == 'product') {
            $this->db->where('attachment_key', $attachmentid);
            $attachment = $this->db->get(db_prefix().'product_files')->row();
            if (!$attachment) {
                show_404();
            }
            if (has_permission('customers', '', 'view') || is_customer_admin($attachment->productid) || is_client_logged_in()) {
                $path = get_upload_path_by_type('files') . $attachment->productid . '/' . $attachment->file_name;
            }
        } else {
