            ...
            if ($this->form_validation->run() !== false) {
				...
				if($this->imap->check_imap($imapconf)){
					if(get_option('email_local')=='yes' ){
						...
						//$ch_data = $this->imap->get_all_mails($staffid,get_option('deal_map'),$imapconf);
						//$table = db_prefix() . 'localmailstorage';
						//$this->db->insert_batch($table, $ch_data);
					}
				}
				$this->session->unset_userdata('message-warning');
				$this->session->unset_userdata('message-info');
				$this->session->unset_userdata('message-danger');
                redirect(admin_url());
            }
        }
