...
class Clients extends AdminController
{
    ...
    public function ajax_client()
    {
        ...
        if ($this->input->is_ajax_request()) {
            if ($id == '') {
                ...
                $data['id']  = $this->clients_model->add($data);
                $data['message']  = _l('added_successfully', _l('client'));
    ...
    public function ajax_project()
    {
        ...
        $data['teamleader'] = get_staff_user_id();
        //$data['status'] = 3;
        ...
        if ($this->input->is_ajax_request()) {
            ...
            $data['result'] = $this->clients_model->getDealsbyID($data);
            echo json_encode($data);
            exit;
        }
        //pre($data);
...
// Merge Contact
    ...
    public function ajax_mergecontact()
    {
        $data = $this->input->post();
//pre($data);
        $data['id']  = $this->clients_model->mergecontact($data);
	...
	public function getDeals() {
        $data['result'] = $this->clients_model->getDeals();
        echo json_encode($data);
        exit;
    }

    public function getContactById() {
        $data['result'] = $this->clients_model->getContactById();
        echo json_encode($data);
        exit;
    }

    public function checkduplicate() {
        $result = $this->clients_model->checkduplicate();
        if($result) {
    ...
    public function checkduplicate_contact() {
        $result = $this->clients_model->checkduplicate_contact();
        //pre($result);
        if($result) {
            $li = '';
            foreach($result as $val) {
                ...
                if($val['email']) {
                    $address .= ' - <span style="color:#008ece;">'.$val['email'].'</span>';
                }
                $li .='<li style="list-style:inside; padding:5px;"><b>'.$val['firstname'].'</b>'.$address.'</li>';
            }
            $message = 'Contact name already exist!, if you still want to create the Contact you can ignore this message.'.'<ul>'.$li.'</ul>';
        } else {
            $message = "no";
        }
...
//pre($data);
			...
			if(!empty($fields1) && $fields1 != 'null'){
				...
				if(!empty($req_fields2)){
					foreach($req_fields2 as $req_field2){
						$data['mandatory_fields1'][$i] = $req_field2;
					}
				}
			}
	 ...
	 public function view_contact($contact_id = '')
    {
        ...
        if ($group != 'contacts' && $contact_id == $this->input->get('contactid')) {
            redirect(admin_url('clients/client/' . $contact_id . '?group=contacts&contactid=' . $contact_id));
        }
    ...
    public function check_duplicate_customer_name()
    {
        if (has_permission('customers', '', 'create')) {
            ...
            $client = $this->clients_model->get_orgdetails_byname($companyName);
            //pre($client);
            $li = '';
            foreach($client as $val) {
                ...
                $li .='<li style="list-style:inside; padding:5px;"><b>'.$val['company'].'</b>'.$address.'</li>';
            }
            $message = _l('company_exists_info', '<b>' . $companyName . '</b>').'<ul>'.$li.'</ul>';
            $response    = [
    ...
    public function form_contact($customer_id=0, $contact_id = '')
    {
        ...
        if ($this->input->post()) {
            ...
            if ($contact_id == '') {
				...
				if(!empty($mandatory_fields) && in_array("clientid", $mandatory_fields)){
					$clientData = $this->clients_model->get_deals_contacts($_POST['clientid']);
				}else{
					$clientData = $this->clients_model->get_deals_contacts_list($_POST['clientid'], ['active' => 1]);
				}
                //pre($clientData);
                $optionsClient = '';
                foreach($clientData as $value) {
                    ...
                    $optionsClient .= '<option value="'.$value['id'].'" '.$selected.'>'.$value['firstname'].'</option>';
                }
				
                echo json_encode([
                    'id'             =>$id,
                    'firstname'             =>$optionsClient,
                    'success'             => $success,
                    'card'             => $card,
                    'message'             => $message,
        ...
        if ($contact_id == '') {
            ...
            $data['deals'] = $this->clients_model->get_dealsbyClientId($data['contact']->userids);
            $data['deals_contact'] = $this->clients_model->get_dealContactsbyClientId($contact_id);
		...
		/*if(empty($data['need_fields']) || !in_array("clientid", $data['need_fields']) || empty($data['mandatory_fields']) || !in_array("clientid", $data['mandatory_fields'])){
			$data['deals'] = $this->clients_model->get_deals();
			
		}*/
		$data['all_clients'] = $this->clients_model->get_clients_list();
    ...
    public function delete_contact($customer_id, $id)
    {
		...
		if(!empty($customer_id)){
			...
			redirect(admin_url('all_contacts'));
		}
    }

    public function restore_contact($id)
    {
        $this->clients_model->restore_contact($id);
    ...
    {
        $this->clients_model->restore_client($id);
