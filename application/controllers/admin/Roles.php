...
//        pre($data);
    ...
    public function modulespermissions($id = '') {
        ...
        if ($this->input->post()) {
            ...
            $success = $this->roles_model->update_modules_permissions($this->input->post());
            if ($success) {
                set_alert('success', _l('updated_successfully', 'Modules Permissions'));
            }
            redirect(admin_url('roles/modulespermissions'));
        }
        $data['modules_permissions'] = $this->roles_model->get_modules_permissions();
//        $data['mp'] = $this->roles_model->get_modules_permissions_active();
        $title = _l('edit', 'Modules Permissions') . ' ';
        $data['title'] = $title;
        $this->load->view('admin/roles/modulespermissions', $data);
    }
