...
class Leads extends AdminController
{
    ...
    public function index($id = '')
    {
		...
		if ($this->session->userdata('switch_kanban_noscroll') == 'true') {
            $data['bodyclass']     = 'kan-ban-body-noscroll';
        }
		if ($this->session->userdata('switch_forecast') == 'true') {
            $data['bodyclass']     = 'leads_switch_forecast';
            $data['forecast']     = $this->leads_model->get_forecast_data();;
        }
		...
		if (!empty($this->session->userdata('pipeline'))) {
			...
			$data['teammembers'] = $this->pipeline_model->getPipelineTeammembers($pipeline);
			$data['teamleaders'] = $this->pipeline_model->getPipelineTeamleaders($pipeline);
			$data['statuses'] = $this->pipeline_model->getPipelineleadstatus($pipeline);
		}
		else {
			$pipeline = '';
			$data['teammembers'] = $this->staff_model->get('', ['active' => 1,'role' => 1]);
			$data['teamleaders'] = $this->staff_model->get('', ['active' => 1,'role' => 2]);
		...
		if(is_admin()) {
			...
			$loggeduserid = $this->session->userdata('staff_user_id');
			$loggeduserdetails = $this->pipeline_model->getUserdetails($loggeduserid);
			if($loggeduserdetails->role == '1') {
				$data['pipelines'] = $this->pipeline_model->getTeammemberpipeline($loggeduserid);
			}
			else if($loggeduserdetails->role == '2') {
				$data['pipelines'] = $this->pipeline_model->getTeamleadpipeline($loggeduserid);
			}
			else {
				$data['pipelines'] = $this->pipeline_model->getPipeline();
			}
            ...
            if (is_array($ids)) {
                foreach ($ids as $id) {
                    if ($this->input->post('mass_delete')) {
                        ...
                        if ($status || $assigned || $last_contact || $visibility) {
                            $update = [];
                            if ($status) {
