...
class Misc extends AdminController
{
	...
	public function send_file_all($cur_id)
    {
        if ($this->input->post('send_file_email')) {
			$this->load->model('emails_model');
			$req_ids = $this->input->post('cur_draft_id');
			$req_ids = rtrim($req_ids,",");
			$all_ids = explode(',',$req_ids);
			if(!empty($all_ids)){
				foreach($all_ids as $all_id1){
					$projects = get_files($all_id1);
					$this->emails_model->add_attachment([
						'attachment' => FCPATH.'uploads/projects/'.$cur_id.'/'.$projects[0]['file_name'],
						'filename'   => $projects[0]['file_name'],
						'type'       => $projects[0]['filetype'],
						'read'       => true,
					]);
					$message = $this->input->post('send_file_message');
					$message = nl2br($message);
    ...
    public function add_productnote($rel_id)
    {
        if ($this->input->post()) {
            $success = $this->misc_model->add_prodnote($this->input->post(), $rel_id);
            if ($success) {
                set_alert('success', _l('added_successfully', _l('note')));
            }
        }
    ...
    public function edit_productnote($id)
    {
        if ($this->input->post()) {
            $success = $this->misc_model->edit_prodnote($this->input->post(), $id);
            echo json_encode([
                'success' => $success,
                'message' => _l('note_updated_successfully'),
            ]);
    ...
    public function delete_productnote($id)
    {
        $success = $this->misc_model->delete_prodnote($id);
    ...
    /* Check if staff emp_id exists / ajax */
    public function staff_emp_id_exists()
    {
        if ($this->input->is_ajax_request()) {
            if ($this->input->post()) {
                // First we need to check if the email is the same
                ...
                if ($member_id != '') {
                    $this->db->where('staffid', $member_id);
                    $_current_email = $this->db->get(db_prefix() . 'staff')->row();
                    if ($_current_email->emp_id == $this->input->post('emp_id')) {
                        echo json_encode(true);
                        die();
                    }
                }
