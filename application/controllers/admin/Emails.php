...
class Emails extends AdminController
{
    ...
    public function index()
    {
        ...
        $data['gdpr'] = $this->emails_model->get([
		 ...
		 $data['reminders'] = $this->emails_model->get([
            'type'     => 'activity_reminder',
            'language' => 'english',
        ]);
		
		$data['proposal_reminders'] = $this->emails_model->get([
            'type'     => 'proposal_reminder',
            'language' => 'english',
        ]);
		$data['target_reminders'] = $this->emails_model->get([
            'type'     => 'target_reminder',
            'language' => 'english',
        ]);
		 $data['activity_show_alerts'] = $this->emails_model->get([
            'type'     => 'activity_show_alert',
            'language' => 'english',
        ]);
		 $data['proposal_show_alerts'] = $this->emails_model->get([
            'type'     => 'proposal_show_alert',
            'language' => 'english',
        ]);
		$data['target_show_alerts'] = $this->emails_model->get([
            'type'     => 'target_show_alert',
            'language' => 'english',
        ]);
    ...
    public function add_email_template($type)
    {
        if (!has_permission('email_templates', '', 'view')) {
            access_denied('email_templates');
        }
        ...
        if ($this->input->post()) {
            ...
            $success = $this->emails_model->add($data);

            if ($success) {
                set_alert('success', _l('added_successfully', _l('email_template')));
            }

            redirect(admin_url('emails/email_template/' . $success));
        }
        ...
        $data['type'] = $type;
        $data['id'] = $id;
        $this->load->view('admin/emails/add_template', $data);
    }
    ...
    public function email_template($id)
    {
        ...
        $data['template'] = $this->emails_model->get_email_template_by_id($id);
		if(!empty($data['template']) && $data['template']->slug == 'proposal-reminder'){
			$data['available_merge_fields'] = $this->app_merge_fields->all();
		}else{
