...
class Proposals extends AdminController
{
    ...
    {
        if ($this->input->post()) {
            $_POST['currency'] = $_POST['currencynew'];
...
//pre($data);
        ...
        //pre($data['products']);
        if($id) {
            $products = $this->products_model->getsales_products($id, $data['proposal']->currency,'proposal');
            //pre($products);
            $data['productscnt'] = count($products);
            $data['products'] = $this->products_model->getitem_price($data['proposal']->currency);
...
//pre($data['products']);
    ...
    public function checkproducts() {
        $check = $this->proposals_model->checkproducts();
        if(!empty($check)) {
            echo '0';
        } else {
            echo '1';
...
//pre($data);
    ...
    public function getTempDetails() {
        $result = $this->proposals_model->getTempDetails();
    ...
    public function get_invoice_convert_data($id)
    {
        ...
        } elseif ($data['proposal']->rel_type == 'customer') {
            $data['customer_id'] = $data['proposal']->rel_id;
            $data['rel_type'] = $data['proposal']->rel_type;
        } else {
            $this->db->where('id', $data['proposal']->rel_id);
            $data['customer_id'] = $this->db->get(db_prefix().'projects')->row()->clientid;
            $data['rel_id'] = $data['proposal']->rel_id;
            $data['rel_type'] = $data['proposal']->rel_type;
        }
    ...
    public function get_related_email_ids()
    {
        $rel_id = $_POST['relid'];
        $rel_type = $_POST['reltype'];
        $search = $_POST['search'];
        //pre($_POST);
        echo json_encode($this->proposals_model->get_related_email_ids($rel_id, $rel_type, $search));
    }

    public function get_deals_primary_email_id()
    {
        $rel_id = $_POST['relid'];
        $rel_type = $_POST['reltype'];
        //pre($_POST);
        echo $this->proposals_model->get_deals_primary_email_id($rel_id, $rel_type);
    }
