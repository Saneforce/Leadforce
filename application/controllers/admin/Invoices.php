...
class Invoices extends AdminController
{
    ...
    public function invoice($id = '')
    {
        ...
        if($id) {
            $products = $this->products_model->getsales_products($id, $data['invoice']->currency,'invoice');
            //pre($products);
            $data['productscnt'] = count($products);
            $data['products'] = $this->products_model->getitem_price($data['invoice']->currency);
        ...
        $data['template'] = $this->emails_model->get(['language' => 'english', 'slug' => 'invoice-send-to-client'], 'row');
        $data['all_templates'] = $this->emails_model->get(['language' => 'english', 'slug' => 'invoice-send-to-client'], 'result_array');
        
        $data['is_invoice'] = true;
        $data['item_for'] = 'invoice';
    ...
    public function get_invoice_data_ajax($id)
    {
       ...
       // $data = prepare_mail_preview_data($template_name, $invoice->clientid);
        ...
        $this->db->where('project_id', $invoice->project_id);
...
//pre($primary_contact_id);
    ...
    public function getTempDetails() {
        $result = $this->invoices_model->getTempDetails();
...
//pre($pdf);
    ...
    public function checkproducts() {
        $check = $this->invoices_model->checkproducts();
        if(!empty($check)) {
            echo '0';
        } else {
            echo '1';
