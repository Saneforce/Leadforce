...
class Invoices_model extends App_Model
{
    ...
    public function get($id = '', $where = [])
    {
        ...
        if (is_numeric($id)) {
            ...
            if ($invoice) {
                ...
                if ($invoice->lead_id != 0) {
                    $this->load->model('leads_model');
                    $invoice->lead_data = $this->leads_model->get($invoice->lead_id);
                }
    ...
    public function add($data, $expense = false)
    {
        //pre($_POST);
        if($data['currencynew']){
            $data['currency'] = $data['currencynew'];
        }
        if($data['projid']){
            $data['project_id'] = $data['projid'];
        }
        if($data['rel_id']){
            $data['project_id'] = $data['rel_id'];
        }
...
//pre($data);
        ...
        if ($insert_id) {
            ...
            $this->load->model('currencies_model');
            if(is_numeric($items['currency'])) {
                $currency = $this->currencies_model->get($items['currency']);
                $cur = $currency->name;
            } else {
                $cur = $items['currency'];
            }

            $this->save_sales_products($items, $insert_id, $cur, 'invoice');
    ...
    public function save_sales_products($data, $id, $currency, $type) {
        ...
        $this->db->where('id', $id);
        $updateid = $this->db->update(db_prefix() . 'invoices', $update);
        return $updateid;
    }
    ...
    public function copy($id)
    {
        ...
        $new_invoice_data['grandtotal']       = $_invoice->total;
        ...
        foreach ($_invoice->items as $item) {
            ...
            $no[] = $key;
            if($item['status'] > 0)
                $new_invoice_data['status_'.$key] = $item['status'];

            $key++;
        }
        $new_invoice_data['no'] = $no;
        $new_invoice_data['tax'] = $tax;
        $new_invoice_data['product'] = $product;
        $new_invoice_data['price'] = $price;
        $new_invoice_data['qty'] = $qty;
        $new_invoice_data['discount'] = $discount;
        $new_invoice_data['total'] = $total;
        
        $new_invoice_data['method'] = $method;
//pre($new_invoice_data);
...
//echo $this->db->last_query(); exit;
        ...
        $this->save_sales_products($items, $id, $cur, 'invoice');
    ...
    public function send_invoice_to_client($id, $template_name = '', $attachpdf = true, $cc = '', $manually = false, $attachStatement = [])
    {
        //pre($_POST);
        $invoice = $this->get($id);
        $email_array = array_map('trim', explode(',', $_POST['email']));
//pre(array_filter($email_array));
        ...
        $emails_sent = [];
        $sent        = false;
        $sent_to     = array_filter($email_array);
        ...
        if (is_array($sent_to) && count($sent_to) > 0) {
            ...
            foreach ($sent_to as $contact_id) {
                if ($contact_id != '') {
                    ...
                    $contact = $this->clients_model->get_contact_by_email($contact_id);
    ...
    public function checkproducts() {
        ...
        $this->db->where('clientid', $_POST['clientid']);
        $this->db->where('project_id', $_POST['rel_id']);
