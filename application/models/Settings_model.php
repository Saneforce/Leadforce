...
{
    private $encrypted_fields = array('smtp_password','company_smtp_password');
    ...
    public function update($data)
    {
        ...
        //pre($data);
        if (isset($data['settings']) && isset($data['settings']['default_currency']) && !empty($data['settings']['default_currency'])){
             $this->db->where('id !=', $data['settings']['default_currency']);
	...
	public function get_reminder_settings($user_id){
	...
	public function update_reminder_settings($user_id,$req_data){
		$table = db_prefix() . 'reminder_settings';
		$res = $this->get_reminder_settings($user_id);
		$cond = array('user_id'=>$user_id);
		$req_data['user_id']   = $user_id;
		if(empty($res)){
			$insert_id = $this->db->insert($table,$req_data);
			if($insert_id)
				return true;
			else
				return false;
			...
			$result = $this->db->update($table, $req_data, $cond);
			if(!empty($result))
				return true;
			else
				return false;
