...
class Clients_model extends App_Model {
    ...
    public function get_projects_contacts($project_id) {
        $this->db->select('contacts_id');
        $this->db->where('project_id', $project_id);
        return $this->db->get(db_prefix() . 'project_contacts')->result_array();
    }

    public function get_projects_primary_contacts($project_id) {
        $this->db->select('contacts_id');
        $this->db->where('project_id', $project_id);
    ...
    public function get_contact($id) {
        return $this->db->query('SELECT a.*,b.company,b.userid as client_id  FROM ' . db_prefix() . 'contacts as a LEFT JOIN ' . db_prefix() . 'clients as b ON b.userid=a.userid  where a.id = '.$id)->row();
    }

    public function get_contact_by_email($id) {
        return $this->db->query('SELECT a.*,b.company,b.userid as client_id  FROM ' . db_prefix() . 'contacts as a LEFT JOIN ' . db_prefix() . 'clients as b ON b.userid=a.userid  where a.email = "'.$id.'"')->row();
    }
    ...
    public function update_contactfromapi($data, $id, $client_request = false) {
        $affectedRows = 0;
        $contact = $this->get_contact($id);
        if (empty($data['password'])) {
            unset($data['password']);
        ...
        $this->db->where('id', $id);
        $update = $this->db->update(db_prefix() . 'contacts', $data);
        if($update) {
            $affectedRows++;
        }
        ...
        if ($affectedRows > 0) {
            ...
            //     $set_password_email_sent = $this->authentication_model->set_password_emailapi($id, $data);
            // }
            // pre($data);
            //log_activity('Contact Updated [ID: ' . $id . ']');
...
//pre($data);
    ...
    public function update_orgbyapi($data, $id, $client_request = false) {
        if (isset($data['update_all_other_transactions'])) {
            $update_all_other_transactions = true;
            unset($data['update_all_other_transactions']);
        }
    ...
    public function checkduplicate_contact() {
        ...
        $where = '';
        if($_POST['pid']) {
            $where .= 'tblcontacts.id != '.$_POST['pid'];
        }
        if($where) {
            $where .= ' AND tblcontacts.firstname = "'.$_POST['name'].'"';
        } else {
            $where .= ' tblcontacts.firstname = "'.$_POST['name'].'"';
        }
        $where = ' WHERE '.$where;
        $sql = 'SELECT tblcontacts.firstname, tblcontacts.phonenumber, tblcontacts.email ,tblclients.company FROM tblcontacts
    ...
    public function getDealsbyID($data = ''){
	 ...
	 public function getDeals(){
        $sql = 'SELECT id, name,company, start_date, deadline, status ,clientid FROM tblprojects
        LEFT JOIN  tblclients ON tblclients.userid = tblprojects.clientid
        WHERE   tblprojects.deleted_status=0 ORDER BY status ASC';
        $query = $this->db->query($sql);
        $result = $query->result_array();
        $select = '';
        foreach($query->result_array() as $val) {
...
//pre($result_cont);
    ...
    public function get_mystaffs_userids($my_staffids) {

        $sql = 'SELECT clientid FROM tblprojects
        WHERE  (' . db_prefix() . 'projects.id IN (SELECT ' . db_prefix() . 'projects.id FROM ' . db_prefix() . 'projects join ' . db_prefix() . 'project_members  on ' . db_prefix() . 'project_members.project_id = ' . db_prefix() . 'projects.id WHERE ' . db_prefix() . 'project_members.staff_id in (' . implode(',',$my_staffids) . ')) OR  ' . db_prefix() . 'projects.teamleader in (' . implode(',',$my_staffids) . ') )';
        $query = $this->db->query($sql);
        $result_cont = $query->result_array();
        //pre($result_cont);
        
        $uids = array();
        foreach($result_cont as $val) {
            $uids[] = $val['clientid'];
        }
