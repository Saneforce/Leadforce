...
class Misc_model extends App_Model
{
    ...
    public function add_product_attachment_to_database($rel_id, $rel_type, $attachment, $external = false)
    {
        $data['dateadded'] = date('Y-m-d H:i:s');
        $data['productid']    = $rel_id;
        if (!isset($attachment[0]['staffid'])) {
            $data['staffid'] = get_staff_user_id();
        } else {
            $data['staffid'] = $attachment[0]['staffid'];
        ...
        $this->db->insert(db_prefix() . 'product_files', $data);
        $insert_id = $this->db->insert_id();
//echo $this->db->last_query();
    ...
    public function get_files($rel_id, $rel_type)
    {
        ...
        return $files = $this->db->get(db_prefix() . 'files')->result_array();
    ...
    public function get_prodnotes($rel_id)
    {
        $this->db->join(db_prefix() . 'staff', db_prefix() . 'staff.staffid=' . db_prefix() . 'product_notes.addedfrom');
        $this->db->where('productid', $rel_id);
        $this->db->order_by('created_date', 'desc');

        $notes = $this->db->get(db_prefix() . 'product_notes')->result_array();
        //pre($notes);
        return hooks()->apply_filters('get_notes', $notes, ['productid' => $rel_id]);
    }

    public function add_prodnote($data, $rel_id)
    {
        ...
        //pre($data);
        $this->db->insert(db_prefix() . 'product_notes', $data);
        $insert_id = $this->db->insert_id();
        if ($insert_id) {
            return $insert_id;
        }
    ...
    public function edit_prodnote($data, $id)
    {
        hooks()->do_action('before_update_note', ['data' => $data, 'id' => $id]);

        $this->db->where('id', $id);
        $this->db->update(db_prefix() . 'product_notes', [
    ...
    public function delete_prodnote($note_id)
    {
        hooks()->do_action('before_delete_note', $note_id);

        $this->db->where('id', $note_id);
        $note = $this->db->get(db_prefix() . 'product_notes')->row();
        if ($note->addedfrom != get_staff_user_id() && !is_admin()) {
            return false;
        }
        $this->db->where('id', $note_id);
        $this->db->delete(db_prefix() . 'product_notes');
        if ($this->db->affected_rows() > 0) {
            return true;
        }
	...
	public function _search_manager($q, $limit = 0)
    {
        $result = [
            'result'         => [],
            'type'           => 'staff',
	...
	public function _search_tags($q, $limit = 0)
    {
        $result = [
            'result'         => [],
            'type'           => 'tags',
            'search_heading' => _l('tags'),
        ];
    ...
    public function _search_projects($q, $limit = 0, $where = false)
    {
		...
		// if(in_array('clientid',$need_fields)){
        ...
        if(get_staff_user_id() != 1) {
            $this->db->where(' (' . db_prefix() . 'projects.id IN (SELECT project_id FROM ' . db_prefix() . 'project_members WHERE staff_id in (' . get_staff_user_id() . ')) OR  ' . db_prefix() . 'projects.teamleader in (' . get_staff_user_id() . ') )');
        }
        if (!startsWith($q, '#')) {
			if(in_array('clientid',$need_fields)){
				$this->db->where('(company LIKE "%' . $q . '%"
