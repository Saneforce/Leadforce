...
class Proposals_model extends App_Model
{
    ...
    public function add($data)
    {
        ...
        if(isset($data['email'])) {
            ...
            $comma = substr($data['email'], -1);
            if($comma == ',') {
                $data['email'] = substr($data['email'], 0, -1);
            }
        }
        ...
        unset($data['tempid']);
        unset($data['email_sub']);
        unset($data['email_template_custom']);
        unset($data['itemid']);
        unset($data['name']);
        unset($data['code']);
        unset($data['categoryid']);
        unset($data['unit_price']);
        unset($data['currencynew']);
        unset($data['change_items']);
        // pr($data);
        // pre($items);
        if ($this->copy == false) {
        ...
        if ($insert_id) {
            ...
            if(is_numeric($items['currency'][0])) {
                $currency = $this->currencies_model->get($items['currency'][0]);
                $cur = $currency->name;
            } else {
                $cur = $items['currency'][0];
            }
            $this->save_sales_products($items, $insert_id, $cur, 'proposal');
...
//echo $save_and_send; exit;
            if ($save_and_send == 1) {
                //pre($_POST);
                $_POST['template_name'] = 'proposal-send-to-customer';
    ...
    public function save_sales_products($data, $id, $currency, $type) {
        //pre($data);
        ...
        foreach($data['product'] as $val) {
            if($val) {
                $insertdate = array();
                $insertdate['rel_id'] = $id;
                $insertdate['rel_type'] = $type;
                $insertdate['qty'] = $data['qty'][$i];
                $insertdate['rate'] = $data['price'][$i];
                ...
                $this->db->insert(db_prefix() . 'itemable', $insertdate);
            }
            $i++;
        }
        $update['total'] = $data['grandtotal'];
        $this->db->where('id', $id);
        $updateid = $this->db->update(db_prefix() . 'proposals', $update);
        return $updateid;
    }
...
//pre($items);
        ...
        $this->save_sales_products($items, $id, $cur, 'proposal');
    ...
    public function checkproducts() {
        $this->db->from(db_prefix() . 'proposals');
        $this->db->where('id', $_POST['project']);
        $this->db->where('rel_type', $_POST['rel_type']);
        $this->db->where('rel_id', $_POST['rel_id']);
    ...
    public function clear_signature($id)
    {
        ...
        if ($proposal) {
            ...
            if (!empty($proposal->signature)) {
                ...
                unlink(get_upload_path_by_type('proposal') . $id . $dbname[0] . '/' . $proposal->signature);
            }
    ...
    {
        //orgName
        $this->db->select('*, (select proposal_to from tblproposals where id = proposalid) as orgName');
    ...
    public function copy($id)
    {
        ...
        $insert_data['grandtotal'] = $insert_data['total'];
        $custom_fields_items     = get_custom_fields('items');
        $key                     = 1;
        $tax = $product = $price = $qty = $discount = $total = $no = array();
        foreach ($proposal->items as $item) {
            ...
            $no[] = $key;
            if($item['status'] > 0)
                $insert_data['status_'.$key] = $item['status'];
    ...
    public function get_relation_data_values($rel_id, $rel_type)
    {
        ...
        if ($rel_type == 'customer') {
            ...
            $custId = $this->db->get(db_prefix() . 'projects')->row();

            $this->db->where('userid', $custId->clientid);
            $_data = $this->db->get(db_prefix() . 'clients')->row();
            $rel_id = $_data->userid;
            
            $this->db->where('project_id', $pid);
            $this->db->where('is_primary', 1);
            $primary_contact_id = $this->db->get(db_prefix() . 'project_contacts')->row();
//pre($primary_contact_id);
            if ($primary_contact_id) {
                $contact     = $this->clients_model->get_contact($primary_contact_id->contacts_id);
                $data->email = $contact->email;
            }
    ...
    public function get_related_email_ids($rel_id, $rel_type, $search)
    {
        
        $data = new StdClass();
        if($_POST['relid'] == '' || $_POST['relid'] == 0) {
            $rel_type = 'customer';
            $rel_id = $_POST['cid'];
        }
        //pre($_POST);
        if ($rel_type == 'customer') {
            ...
            $_data = $this->db->get(db_prefix() . 'contacts')->result_array();
            $email_ids = array();
            $i=0;
            foreach($_data as $emails) {
                if(!empty($emails['email'])) {
                    $email_ids[$i]['label'] = $emails['email'];
                    $email_ids[$i]['value'] = $emails['email'];
                    $email_ids[$i]['id'] = $emails['id'];
                    $i++;
                }
            }
            return $email_ids;
...
exit;
    ...
    public function get_deals_primary_email_id($rel_id, $rel_type)
    {
        if($_POST['relid'] == '') {
            $rel_type = 'customer';
            $rel_id = $_POST['cid'];
        ...
        if ($rel_type == 'customer') {
            $this->db->where('userid', $rel_id);
            $_data = $this->db->get(db_prefix() . 'contacts')->row();
            //pre($_data);
            if($_data) {
                return $_data->email;
            }
    ...
    public function send_proposal_to_email($id, $attachpdf = true, $cc = '')
    {
        ...
        $proposal = $this->get($id);
        $emails = explode(',',$proposal->email);
        foreach($emails as $val) {
            $proposal->email = trim($val);
            //pre($proposal);
...
//pre($sent);
    ...
    public function getTempDetails() {
        $temp_id = $_POST['temp_id'];
        $this->db->where('emailtemplateid', $temp_id);
        $_data = $this->db->get(db_prefix() . 'emailtemplates')->row();
        $result = array();
        $result['subject'] = $_data->subject;
        $result['message'] = $_data->message;
        $result['tempid'] = $temp_id;
        return $result;
    }
}
