...
class Emails_model extends App_Model
{
    ...
    public function add($data)
    {
        ...
        //pre($_data);
        $this->db->insert(db_prefix() . 'emailtemplates', $_data);
        $insert_id = $this->db->insert_id();
        $main_template = $this->get_email_template_by_id($insert_id);

        if ($main_template) {
            log_activity('Email Template Inserted [' . $main_template->name . ']');
     ...
     */
    public function update($data, $tempid)
    {
        //pre($data);
        $data['plaintext'] = isset($data['plaintext']) ? 1 : 0;
