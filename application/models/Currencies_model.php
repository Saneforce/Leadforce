...
class Currencies_model extends App_Model
{
    ...
    public function get_unit_currencies($id = false)
    {
        $currencies = "";
        if (is_numeric($id)) {
            $this->db->select('currency');
            $this->db->where('productid', $id);
            $currency = $this->db->get(db_prefix() . 'unit_price')->result_array();
            return $currency;
        }
    ...
    public function getAllCurrency() {
        ...
        return $currencies = $this->db->get(db_prefix() . 'currencies')->result_array();
    }

    public function getConvRate() {
        $this->db->where('currency_from', $_POST['name']);
    ...
    public function edit($data)
    {
        ...
        $currency = $this->db->get(db_prefix() . 'currencies')->row();
        $data['name'] = strtoupper($data['name']);
        $this->db->where('currency', $currency->name);
        $result = $this->db->get(db_prefix() . 'project_products')->row();
        if($result) {
            $update['crate'] = $data['crate'];
            $this->db->where('id', $currencyid);
            $this->db->update(db_prefix() . 'currencies', $update);
            return 2;
        } else {
            $this->db->where('id', $currencyid);
    ...
    {
        $this->db->where('id', $id);
        $result = $this->db->get(db_prefix() . 'currencies')->row();
        
        if($result) {
            $this->db->where('currency', $result->name);
            $pp = $this->db->get(db_prefix() . 'project_products')->row();
            if($pp) {
                return false;
            }
        }
    ...
    public function editConversionRate($data) {
        $this->db->select('*');
        $this->db->where('currency_from', $data['currency_from']);
        $this->db->where('currency_to', $data['currency_to']);
        $result = $this->db->get(db_prefix() . 'conversion_rate')->result_array();
        //pre($result);
        if($result) {
            $this->db->where('currency_from', $result[0]['currency_from']);
            $this->db->where('currency_to', $result[0]['currency_to']);
            $this->db->update(db_prefix() . 'conversion_rate', $data);
        } else {
            $this->db->insert(db_prefix() . 'conversion_rate', $data);
        }
        return true;
    }
}
