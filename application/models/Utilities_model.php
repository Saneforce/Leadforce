...
class Utilities_model extends App_Model
{
    ...
    {
		$dashboard_form_data = $this->session->userdata('dashboard_form_data');
        ...
        if (get_option('show_tasks_on_calendar') == 1 && !$ff || $ff && array_key_exists('tasks', $filters)) {
            if ($client_data && !$has_contact_permission_projects) {
				...
				if (isset($dashboard_form_data['member']) && !empty($dashboard_form_data['member'])) {
                    $this->db->where('(id IN (SELECT taskid FROM ' . db_prefix() . 'task_assigned WHERE staffid in (' . $dashboard_form_data['member'] . ')))');
                } else {
                    if($my_staffids){
                        $this->db->where('(id IN (SELECT taskid FROM ' . db_prefix() . 'task_assigned WHERE staffid in (' . get_staff_user_id() . ')) OR id IN (SELECT taskid FROM ' . db_prefix() . 'task_assigned WHERE staffid in (' . implode(',',$my_staffids) . ')))');
                    } else {
                        if($admin == 0) {
                            $this->db->where('(id IN (SELECT taskid FROM ' . db_prefix() . 'task_assigned WHERE staffid in (' . get_staff_user_id() . ')))');
                        }
                    }
                }
				
				if (isset($dashboard_form_data['pipelines']) && !empty($dashboard_form_data['pipelines'])) {
					$this->db->where('rel_type', 'project');
                    $this->db->where('(rel_id IN (SELECT id FROM ' . db_prefix() . 'projects WHERE pipeline_id in (' . $dashboard_form_data['pipelines'] . ')))');
                }
                ...
                foreach ($tasks as $task) {
                    ...
                    if (!empty($task['rel_id']) && !$client_data) {
                        $datetime = date('h:i A ', strtotime($task['date']));
                        $rel_showcase = ' (' . $task['rel_name'] . ') - '.$datetime;
                    }
                    ...
                    if (!$client_data) {
                        $task['onclick'] = 'edit_task(' . $task['id'] . '); return false;';
                        $task['url']     = '#';
                    } else {
        ...
        //calendar_project
        if ((get_option('show_projects_on_calendar') == 1 && !$ff || $ff && array_key_exists('projects', $filters)) && 0) {
			...
			if (isset($dashboard_form_data['member']) && !empty($dashboard_form_data['member'])) {
				$this->db->where('id IN (SELECT project_id FROM ' . db_prefix() . 'project_members WHERE staff_id=' . $dashboard_form_data['member'] . ')');
			}
			
			if (isset($dashboard_form_data['status']) && !empty($dashboard_form_data['status'])) {
				$this->db->where_in('status', $dashboard_form_data['status']);
			}

			if (isset($dashboard_form_data['pipelines']) && !empty($dashboard_form_data['pipelines'])) {
				$this->db->where_in('pipeline_id', $dashboard_form_data['pipelines']);
			}
