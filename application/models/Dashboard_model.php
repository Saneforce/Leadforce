    ...
    public function projects_status_stats()
    {
        ...
        foreach ($statuses as $status) {
			...
			if (isset($_POST['member']) && !empty($_POST['member'])) {
                //$sql .= ' AND (teamleader=' . $_POST['member'] . ' OR id IN (SELECT project_id FROM ' . db_prefix() . 'project_members WHERE staff_id=' . $_POST['member'] . '))';
                $sql .= ' AND (' . db_prefix() . 'projects.id IN (SELECT ' . db_prefix() . 'projects.id FROM ' . db_prefix() . 'projects join ' . db_prefix() . 'project_members  on ' . db_prefix() . 'project_members.project_id = ' . db_prefix() . 'projects.id WHERE ' . db_prefix() . 'project_members.staff_id in (' . $_POST['member'] . ')) OR  ' . db_prefix() . 'projects.teamleader in (' . $_POST['member'] . ') )';
            } else {
                if($my_staffids){
                    $sql .= ' AND (' . db_prefix() . 'projects.id IN (SELECT ' . db_prefix() . 'projects.id FROM ' . db_prefix() . 'projects join ' . db_prefix() . 'project_members  on ' . db_prefix() . 'project_members.project_id = ' . db_prefix() . 'projects.id WHERE ' . db_prefix() . 'project_members.staff_id in (' . implode(',',$my_staffids) . ')) OR  ' . db_prefix() . 'projects.teamleader in (' . implode(',',$my_staffids) . ') )';
                }
            }
            
            if (isset($_POST['pipelines']) && !empty($_POST['pipelines'])) {
				$sql .= ' AND pipeline_id = ' . $_POST['pipelines'] . '';
            }
            $sql .= ' AND tblprojects.deleted_status = 0';
