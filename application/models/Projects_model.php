...
class Projects_model extends App_Model
{
    ...
    public function get_kanbanproject_statuses()
    {
    ...
    public function get_project_statuses($pipelines = 0)
    {
        $npstatus = array();
        if($pipelines != 0){
            $this->db->where('id', $pipelines);
            $npstatus = $this->db->get(db_prefix() . 'pipeline')->row()->status;
            if(!empty($npstatus)){
                $npstatus = explode(',',$npstatus);
            }else{
                $npstatus = array();
            }
        ...
        $statusesq =  $this->db->select(array('statusorder as order','statusorder','color','id','name','isdefault  as filter_default','isdefault'));
        if(count($npstatus)){
            $statusesq->where_in('id', $npstatus);
        }
        $this->db->order_by("statusorder", "asc");
        $statuses = $statusesq->get(db_prefix() . 'projects_status')->result_array();
    ...
    public function get_project_statuses12()
    {
    ...
    public function get_distinct_projects_members()
    {
        if(is_admin(get_staff_user_id())) {
            ...
            return $this->db->query('SELECT staffid as staff_id, firstname, lastname FROM tblstaff where staffid IN (select teamleader from tblprojects where teamleader != "") or staffid IN (select staff_id from tblproject_members where project_id IN (select project_id from tblproject_members)) group by staff_id order by firstname ASC')->result_array();
        } else {
            return $this->db->query('SELECT staffid as staff_id, firstname, lastname FROM tblstaff where staffid IN (select staff_id from tblproject_members where project_id IN (select project_id from tblproject_members where staff_id="'.get_staff_user_id().'")) group by staff_id order by firstname ASC')->result_array();
        }
    }
    ...
    public function conversionrate($name,$to) {
        $this->db->where('currency_from', $name);
        $this->db->where('currency_to', $to);
        return $currencies = $this->db->get(db_prefix() . 'conversion_rate')->result_array();
    }

    public function get_pipeline_prob($stage) {
        $this->db->select('progress');
        $this->db->where('id', $stage);
        return $progress = $this->db->get(db_prefix() . 'projects_status')->row();
    ...
    public function getproject($id = '', $where = [])
    {
        $this->db->where($where);
        if (is_numeric($id)) {
            $this->db->select('*, (select CONCAT(' . db_prefix() . 'staff.firstname, \' \', ' . db_prefix() . 'staff.lastname) as fullname from tblstaff where staffid=' . db_prefix() . 'projects.teamleader) as owner_name');
            $this->db->where('id', $id);
            $project = $this->db->get(db_prefix() . 'projects')->row();
            if ($project) {
                $project->shared_vault_entries = $this->clients_model->get_vault_entries($project->clientid, ['share_in_projects' => 1]);
    ...
    public function get_notes($project_id)
    {
    ...
    public function get_notes_byid($id)
    {
        $this->db->where('id', $id);
        return $this->db->get(db_prefix() . 'project_notes')->row();
    }
    ...
    {
        $this->db->where('id', $id);
        $file = $this->db->get(db_prefix() . 'project_notes')->row();
           
        $this->db->where('id', $id);
        $this->db->delete(db_prefix() . 'project_notes');

        $this->log_activity($project_id, 'project_activity_project_notes_removed', '');
        return true;
    }
    ...
    public function get_all_projects_gantt_data($filters = [])
    {
        ...
        foreach ($statuses as $status) {
            ...
            $gsearch   = $this->input->get('gsearch');
            if(!empty($gsearch)){
            ...
            if($my_staffids){
                $this->db->where('id IN (SELECT project_id FROM ' . db_prefix() . 'project_members WHERE staff_id in (' . implode(',',$my_staffids) . '))');
            }
    ...
    public function add($data,$products=false,$project_contacts=false,$primary_contact=false)
    {
        ...
        //$data['deadline'] = to_sql_date($data['start_date']);
    ...
    public function checkdeadline($date, $id) {
        $project = $this->get($id);
    ...
    public function update($data, $id)
    {
        ...
        if(isset($data['clientid'])) {
            if($data['clientid'] == $original_project->clientid) {
                return false;
                exit;
            } else {
                $contactss = $this->clients_model->get_deals_contacts($data['clientid']);
                if(isset($data)) {
                    $first_cont = $contactss[0]['id'];
                    $this->db->where('project_id', $id);
                    $this->db->delete(db_prefix() . 'project_contacts');
                    if ($this->db->affected_rows() > 0) {
                        $this->db->insert(db_prefix() . 'project_contacts', [
                            'project_id' => $id,
                            'contacts_id'   => $first_cont,
                            'is_primary' => '1',
                        ]);
                    }
                }
        ...
        if(isset($data['pipeline_id'])) {
            if($data['pipeline_id'] == $original_project->pipeline_id) {
                return false;
                exit;
            } else {
                $pipestatus = $this->pipeline_model->getPipelineleadstatus($data['pipeline_id']);
                if(isset($pipestatus)) {
                    $updateProj['status'] = $pipestatus[0]['id'];
                } else {
                    $updateProj['status'] = 0;
                }
                $this->db->where('id', $id);
                $this->db->update(db_prefix() . 'projects', $updateProj);
            }
        }
        ...
        if (isset($data['project_members']) && !empty($data['project_members'])) {
            $project_members = $data['project_members'];
            unset($data['project_members']);
        }
        $_pm = [];
        ...
        $data['progress'] = $this->getprogressstatus($data['status']);
            ...
            if ($original_project->status != $data['status']) {
                ...
                if(isset($data['status'])){
                    if ($data['status'] == 4) {
                        ...
                        $this->log_activity($id, 'project_status_updated', '<b>' . $this->get_projects_status_name($data['status']) . '</b>');
                    }
                }
    ...
    public function getprogressstatus($status_id)
    {
        $this->db->select('progress');
        $this->db->where('id', $status_id);
        return $this->db->get(db_prefix() . 'projects_status')->row()->progress;
    }

    public function get_projects_status_name($status_id)
    {
        $this->db->select('name');
        $this->db->where('id', $status_id);
        return $this->db->get(db_prefix() . 'projects_status')->row()->name;
    }
	...
	public function mark_as_won_loss_reopen($data)
    {
        ...
        if($datasave['stage_of'] == 2){
            ...
            $datasave['loss_reason'] = $data['loss_reason'];
        }else{
            $datasave['loss_remark'] = '';
            $datasave['loss_reason'] = '';
        }
        $datasave['stage_on'] = date('Y-m-d');
        $this->db->where('id', $data['project_id']);
    ...
    public function add_primary_contacts($primary_contact, $id){
        $this->db->where('project_id', $id);
        $this->db->update(db_prefix() . 'project_contacts', [
            'is_primary' => 0
        ]);
        $this->db->where('project_id', $id);
        $this->db->where('contacts_id', $primary_contact);
        $this->db->update(db_prefix() . 'project_contacts', [
            'is_primary' => 1
        ]);
        return true;
    ...
    public function add_edit_contacts($data, $id)
    {
        ...
        $new_project_contacts_to_receive_email = [];
        $this->db->select('name,clientid');
        $this->db->where('id', $id);
        $project      = $this->db->get(db_prefix() . 'projects')->row();
        $project_name = $project->name;
        $client_id    = $project->clientid;
        $project_contacts_in = $this->get_project_contacts($id);
        
        if (sizeof($project_contacts_in) > 0) {
            foreach ($project_contacts_in as $project_contact) {
                
                if (isset($project_contacts)) {
                    //if (!in_array($project_contact['contacts_id'], $project_contacts)) {
                        
                        $this->db->where('project_id', $id);
                        $this->db->where('contacts_id', $project_contact['contacts_id']);
                        $this->db->delete(db_prefix() . 'project_contacts');
                        if ($this->db->affected_rows() > 0) {
                            $this->db->where('contacts_id', $project_contact['contacts_id']);
                            $this->db->where('project_id', $id);
                            ...
                            $this->log_activity($id, 'project_activity_removed_team_contact', get_contact_full_name($project_contact['staff_id']));
                            $affectedRows++;
                        }
                    //} 
                } else {
            ...
            if (isset($project_contacts)) {
                $notifiedUsers = [];
                foreach ($project_contacts as $staff_id) {
                    $this->db->where('project_id', $id);
                    $this->db->where('contacts_id', $staff_id);
                    $_exists = $this->db->get(db_prefix() . 'project_contacts')->row();
                    if (!$_exists) {
                        if (empty($staff_id)) {
                            continue;
                        }
                       
                        if($staff_id == $data['primary_contact']) {
                           
                            $this->db->insert(db_prefix() . 'project_contacts', [
                                'project_id' => $id,
                                'contacts_id'   => $staff_id,
                                'is_primary' => '1',
                            ]);
                        } else {
                            $this->db->insert(db_prefix() . 'project_contacts', [
                        ...
                        if ($this->db->affected_rows() > 0) {
                            ...
                            //     $notified = add_notification([
                            //         'fromuserid'      => get_staff_user_id(),
                            //         'description'     => 'not_staff_added_as_project_contact',
                            //         'link'            => 'projects/view/' . $id,
                            //         'touserid'        => $staff_id,
                            //         'additional_data' => serialize([
                            //             $project_name,
                            //         ]),
                            //     ]);
                            //     array_push($new_project_contacts_to_receive_email, $staff_id);
                            //     if ($notified) {
                            //         array_push($notifiedUsers, $staff_id);
                            //     }
                            // }
                            ...
                            // $this->log_activity($id, 'project_activity_added_team_contact', get_contact_full_name($staff_id));
                            $affectedRows++;
                        }
                    }
                }
    ...
    public function get_project_members($id,$projectdetails = array())
    {
       
        $pmids = array();
        // if(isset($projectdetails['pipeline_id']) && !empty($projectdetails['pipeline_id'])){
        //     $pmidsq = $this->db->where('id', $projectdetails['pipeline_id'])->get(db_prefix() . 'pipeline')->row()->teammembers;
        //     $pmids = explode(',',$pmidsq );
        // }
    ...
    public function remove_team_contact($project_id, $contacts_id)
    {
        $this->db->where('project_id', $project_id);
        $this->db->where('contacts_id', $contacts_id);
        $this->db->delete(db_prefix() . 'project_contacts');
        if ($this->db->affected_rows() > 0) {
            $this->log_activity($project_id, 'project_activity_removed_team_contact', get_contact_full_name($contacts_id));
            return true;
        }
...
//echo $project_id;
  //      pr($data);
        ...
        $sdate = strtotime($data['start_date1']);
        if($data['deadline1']) {
            $edate = strtotime($data['deadline1']);
            if($edate < $sdate) {
                if($project_id) {
        ...
        $project_contacts = [];
        foreach($data['project_contacts1'] as $pc) {
            $project_contacts['project_contacts'][] = $pc;
        }
        //pre($project_contacts);
        $primary_contact = $data['primary_contact1'];
        
        $_new_data['description'] = $data['description1'];
        
        
        unset($_new_data['id']);
        $_new_data['clientid'] = $data['clientid'];
        //unset($_new_data['clientid_copy_project']);
        $_new_data['teamleader'] = $data['teamleader1'];
        
        $_new_data['start_date'] = to_sql_date($data['start_date1']);

        
        $_new_data['pipeline_id'] = $data['pipeid'];
        $_new_data['status'] = $data['status1'];
        if ($data['deadline1']) {
            $_new_data['deadline'] = to_sql_date($data['deadline1']);
        } else {
        ...
        if ($id) {
            $this->add_edit_contacts($project_contacts, $id);
            $this->add_primary_contacts($primary_contact, $id);
            if (isset($data['project_members'])) {
                $_pm['project_members'] = $data['project_members'];
    ...
    public function save_note($data, $project_id)
    {
        ...
        //     if ($this->db->affected_rows() > 0) {
        //         $staff_id = get_staff_user_id();
        //         $this->log_activity($project_id, 'project_activity_updated_a_new_note', get_contact_full_name($staff_id));
        //         return true;
        //     }
    ...
    public function edit_note($data,$project_id)
    {
        // Check if the note exists for this project;
        $this->db->where('id', $data['id']);
        $notes = $this->db->get(db_prefix() . 'project_notes')->row();
        if ($notes) {
            ...
            if ($this->db->affected_rows() > 0) {
                $staff_id = get_staff_user_id();
                $this->log_activity($project_id, 'project_activity_updated_a_new_note', '');
                return true;
            }
            return false;
        }
	 ...
	 public function get_activity($id = '', $limit = '', $only_project_members_activity = false)
    {
        ...
        $checkadmin = $this->db->get(db_prefix() . 'staff')->row();
	...
	public function count_email($staffid='',$id = '', $offset = '', $limit = ''){
		 if (is_numeric($id)) {
            $this->db->where('project_id', $id);
        }
    ...
    public function get_email($id = '', $offset = '', $limit = '')
    {
       ...
       if(!empty($offset)){
        ...
        foreach ($emails as $email1) {
			$req_mail = array();
			$j = 0;
			if(!empty(json_decode($email1['mail_to']))){
				foreach(json_decode($email1['mail_to']) as $ch_email){
					$req_mail[$j] = $ch_email->email;
					$j++;
				}
			}
            $activities[$i]['id']        = $email1['id'];
            $activities[$i]['mail_by']   = $email1['mail_by'];
            $activities[$i]['message_id']= $email1['message_id'];
            $activities[$i]['uid']       = $email1['uid'];
            $activities[$i]['folder']    = $email1['folder'];
            $activities[$i]['staff_id']    = $email1['staff_id'];
            $activities[$i]['from']      = $email1['from_email'];
            $activities[$i]['to'] 		 = implode(",",$req_mail);
            $activities[$i]['subject']   = $email1['subject'];
            $activities[$i]['message']   = substr($email1['body_plain'],0,30).'...';
            $activities[$i]['date']   	 = $email1['date'];
            $activities[$i]['attachements']   	 = $email1['attachements'];
            $i++;
        }
    ...
    public function checkduplicate() {
        ...
        // return $result = $this->db->get(db_prefix() . 'projects')->result_array();

        $where = '';
        if($_POST['pid']) {
            $where .= 'tblprojects.id != '.$_POST['pid'];
        }
        if($where) {
            $where .= ' AND tblprojects.name = "'.$_POST['name'].'"';
        } else {
            $where .= ' tblprojects.name = "'.$_POST['name'].'"';
        }
        $where = ' WHERE '.$where;
        $sql = 'SELECT tblprojects.name,tblclients.company FROM tblprojects
	...
	public function do_kanban_query_count($status, $search = '', $page = 1, $sort = [], $count = false) {
        
        // ROle based records
		$my_id = get_staff_user_id();
		if(empty($my_id)){
			$_SESSION['member'] = $my_id;
		}
        $my_staffids = $this->staff_model->get_my_staffids();
    ...
    public function do_kanban_query($status, $search = '', $page = 1, $sort = [], $count = false) {
        ...
        //         $this->db->where('(' . db_prefix() . 'projects.name LIKE "%' . $search . '%" OR ' . db_prefix() . 'projects.email LIKE "%' . $search . '%" OR ' . db_prefix() . 'projects.phonenumber LIKE "%' . $search . '%" OR ' . db_prefix() . 'projects.company LIKE "%' . $search . '%" OR CONCAT(' . db_prefix() . 'staff.firstname, \' \', ' . db_prefix() . 'staff.lastname) LIKE "%' . $search . '%")');
        //     } else {
        ...
        // $gsearch   = $this->input->get('gsearch');
        // if(empty($gsearch)){
        //     $gsearch = $this->session->userdata('gsearch');
        // }
        // if(!empty($gsearch)){
        //     $this->db->where(db_prefix() . 'projects.id IN (SELECT id FROM ' . db_prefix() . 'projects WHERE name like "%' . $gsearch . '%")');
        // } 
        ...
        // if ($_SESSION['member']) {
        //     $this->db->where('(' . db_prefix() . 'projects.teamleader = ' . $_SESSION['member'] .')');
        // }
	...
	public function do_kanban_forecast_query($intervel, $which, $year, $count = false, $search = '', $page = 1, $sort = []) {
        ...
        $default_projects_kanban_sort_type = get_option('default_projects_kanban_sort_type');
        $has_permission_view = has_permission('projects', '', 'view');
        $this->db->select("".db_prefix() . 'projects.id` as id,count('.db_prefix() . 'tasks.rel_id) as taskscount,'.db_prefix() . 'projects.deadline` as deadline,projects.start_date as start_date,'.db_prefix() . 'clients.company as title ,' . db_prefix() . 'clients.website, ' . db_prefix() . 'clients.address, ' . db_prefix() . 'clients.city, ' . db_prefix() . 'clients.state, ' . db_prefix() . 'clients.country, ' . db_prefix() . 'clients.zip, ' . db_prefix() . 'projects.name as project_name,' . db_prefix() . 'pipeline.name as pipeline_name,' . db_prefix() . 'clients.company,' . db_prefix() . 'projects.status, (SELECT COUNT(id) FROM ' . db_prefix() . 'files WHERE rel_id=' . db_prefix() . 'projects.id AND rel_type="project") as total_files, (SELECT COUNT(id) FROM ' . db_prefix() . 'notes WHERE rel_id=' . db_prefix() . 'projects.id AND rel_type="project") as total_notes,(SELECT GROUP_CONCAT(name SEPARATOR ",") FROM ' . db_prefix() . 'taggables JOIN ' . db_prefix() . 'tags ON ' . db_prefix() . 'taggables.tag_id = ' . db_prefix() . 'tags.id WHERE rel_id = ' . db_prefix() . 'projects.id and rel_type="project" ORDER by tag_order ASC) as tags, '.db_prefix() . 'projects.project_cost as project_cost,'.db_prefix() . 'projects.project_currency as project_currency,'.db_prefix() . 'projects.stage_of as stage_of');
        $this->db->from(db_prefix() . 'projects');
        $this->db->join(db_prefix() . 'project_members', db_prefix() . 'project_members.project_id=' . db_prefix() . 'projects.id', 'left');
		$this->db->join(db_prefix() . 'tasks', db_prefix() . 'tasks.rel_id=' . db_prefix() . 'projects.id and ' . db_prefix() . 'tasks.rel_type="project"', 'left');
        $this->db->join(db_prefix() . 'clients', db_prefix() . 'clients.userid=' . db_prefix() . 'projects.clientid', 'left');
        $this->db->join(db_prefix() . 'staff', db_prefix() . 'staff.staffid=' . db_prefix() . 'project_members.staff_id', 'left');
        $this->db->join(db_prefix() . 'pipeline', db_prefix() . 'pipeline.id=' . db_prefix() . 'projects.pipeline_id', 'left');
        if($_SESSION['forecast_showby']) {
            if($_SESSION['forecast_showby'] == 'close date') {
                if($intervel == 'quarter') {
                    $this->db->where('QUARTER(tblprojects.deadline)',$which);
                    $this->db->where('YEAR(tblprojects.deadline)',$year);
                }
                if($intervel == 'month') {
                    $this->db->where('MONTH(tblprojects.deadline)',$which);
                    $this->db->where('YEAR(tblprojects.deadline)',$year);
                }
                if($intervel == 'week') {
                    if($which == 52) {
                        $this->db->where('((WEEK(tblprojects.deadline) = 52 AND YEAR(tblprojects.deadline) = '.$year.') OR (WEEK(tblprojects.deadline) = 0 AND YEAR(tblprojects.deadline) = '.($year+1).'))');
                    } else {
                        $this->db->where('WEEK(tblprojects.deadline)',$which);
                        $this->db->where('YEAR(tblprojects.deadline)',$year);
                    }
                }
                
            }
            if($_SESSION['forecast_showby'] == 'created date') {
                if($intervel == 'quarter') {
                    $this->db->where('QUARTER(tblprojects.start_date)',$which);
                    $this->db->where('YEAR(tblprojects.start_date)',$year);
                }
                if($intervel == 'month') {
                    $this->db->where('MONTH(tblprojects.start_date)',$which);
                    $this->db->where('YEAR(tblprojects.start_date)',$year);
                }
                if($intervel == 'week') {
                    if($which == 52) {
                        $this->db->where('((WEEK(tblprojects.start_date) = 52 AND YEAR(tblprojects.start_date) = '.$year.') OR (WEEK(tblprojects.start_date) = 0 AND YEAR(tblprojects.start_date) = '.($year+1).'))');
                    } else {
                        $this->db->where('WEEK(tblprojects.start_date)',$which);
                        $this->db->where('YEAR(tblprojects.start_date)',$year);
                    }
                }
            }
        }
        $this->db->where('(tblprojects.stage_of = 0 or tblprojects.stage_of = 1)');
        $this->db->where(db_prefix() . 'projects.deleted_status', 0);
        if (empty($_SESSION['member'])) {
            if ($my_staffids){
                $this->db->where('((' . db_prefix() . 'projects.id IN (SELECT ' . db_prefix() . 'projects.id FROM ' . db_prefix() . 'projects join ' . db_prefix() . 'project_members  on ' . db_prefix() . 'project_members.project_id = ' . db_prefix() . 'projects.id WHERE ' . db_prefix() . 'project_members.staff_id in (' . implode(',',$my_staffids) . '))) OR  ' . db_prefix() . 'projects.teamleader in (' . implode(',',$my_staffids) . '))');
        ...
        if ($_SESSION['forecast_orderby'] == 'open deal') {
            $this->db->order_by('tblprojects.stage_of', 'asc');
        } else {
            $this->db->order_by('tblprojects.stage_of', 'desc');
        }
        $this->db->group_by(db_prefix() . 'projects.id`');
    ...
    public function do_kanban_forecast_query_all($intervel, $which, $year, $count = false, $search = '', $page = 1, $sort = []) {
                //pre($_SESSION);
        // ROle based records
        $my_staffids = $this->staff_model->get_my_staffids();
        $limit = get_option('projects_kanban_limit');
    ...
    { 
     ...
     /**
     * Update canban project status when drag and drop
    ...
    public function add_dealfromapi($data)
    {
        if (isset($data['notify_project_members_status_change'])) {
            unset($data['notify_project_members_status_change']);
        }
        ...
        $data['deadline'] = date('Y-m-d',strtotime($data['start_date']));
        ...
        if ($insert_id) {
            ...
            if (isset($project_members)) {
                $_pm['project_members'] = $project_members;
                $this->add_edit_members_byapi($_pm, $insert_id);
            }
            $original_settings = $this->get_settings();
            if (isset($project_settings)) {
                $_settings = [];
            ...
            $dataactivity['fullname']   = $staffname;
            $dataactivity['description_key']     = 'project_activity_created';
            $dataactivity['additional_data']     = '';
            $dataactivity['visible_to_customer'] = 1;
            $dataactivity['project_id']          = $insert_id;
    ...
    public function add_edit_members_byapi($data, $id)
    {
        $affectedRows = 0;
        if (isset($data['project_members'])) {
            $project_members = $data['project_members'];
                ...
                if (isset($project_members)) {
                    if (!in_array($project_member['staff_id'], $project_members)) {
                        ...
                        if ($this->db->affected_rows() > 0) {
                            ...
                            $this->db->where('staffid', $project_member['staff_id']);
                            $contactdetail = $this->db->select('firstname,lastname')->from(db_prefix() . 'staff')->get()->row();
                            $contactname = html_escape($contactdetail ? $contactdetail->firstname . ' ' . $contactdetail->lastname : '');
                            
                            $dataactivity = array();
                            $dataactivity['contact_id'] = 0;
                            $dataactivity['staff_id']   = get_staff_user_id();
                            $dataactivity['fullname']   = $staffname;
                            $dataactivity['description_key']     = 'project_activity_removed_team_member';
                            $dataactivity['additional_data']     = $contactname;
                            $dataactivity['visible_to_customer'] = 1;
                            $dataactivity['project_id']          = $id;
                            $dataactivity['dateadded']           = date('Y-m-d H:i:s');
                ...
                foreach ($project_members as $staff_id) {
                    ...
                    if (!$_exists) {
                        ...
                        if ($this->db->affected_rows() > 0) {
                            ...
                            $dataactivity['staff_id']   = get_staff_user_id();
                            $dataactivity['fullname']   = $staffname;
                            $dataactivity['description_key']     = 'project_activity_added_team_member';
                            $dataactivity['additional_data']     = $contactname;
                            $dataactivity['visible_to_customer'] = 1;
                            $dataactivity['project_id']          = $id;
                            $dataactivity['dateadded']           = date('Y-m-d H:i:s');
    ...
    public function add_edit_contactsapi($data, $id)
    {
        $affectedRows = 0;
        if (isset($data['project_contacts'])) {
            $project_contacts = $data['project_contacts'];
            ...
            foreach ($project_contacts_in as $project_contact) {
                if (isset($project_contacts)) {
                    ...
                    if (!in_array($project_contact['contacts_id'], $project_contacts)) {
                        ...
                        $this->db->where('project_id', $id);
                        $this->db->where('contacts_id', $project_contact['staff_id']);
                        $this->db->delete(db_prefix() . 'project_contacts');
                        if ($this->db->affected_rows() > 0) {
                            $this->db->where('contacts_id', $project_contact['staff_id']);
                            $this->db->where('project_id', $id);
                            ...
                            $this->db->where('id', $project_contact['staff_id']);
                            $contactdetail = $this->db->select('firstname,lastname')->from(db_prefix() . 'contacts')->get()->row();
                            $contactname = html_escape($contactdetail ? $contactdetail->firstname . ' ' . $contactdetail->lastname : '');
                            ...
                            $dataactivity['staff_id']   = get_staff_user_id();
                            $dataactivity['fullname']   = $staffname;
                            $dataactivity['description_key']     = 'project_activity_removed_team_contact';
                            $dataactivity['additional_data']     = $contactname;
                            $dataactivity['visible_to_customer'] = 1;
                            $dataactivity['project_id']          = $id;
                            $dataactivity['dateadded']           = date('Y-m-d H:i:s');
                ...
                foreach ($project_contacts as $staff_id) {
                    ...
                    if (!$_exists) {
                        ...
                        if ($this->db->affected_rows() > 0) {
                            ...
                            $dataactivity['staff_id']   = get_staff_user_id();
                            $dataactivity['fullname']   = $staffname;
                            $dataactivity['description_key']     = 'project_activity_added_team_contact';
                            $dataactivity['additional_data']     = $contactname;
                            $dataactivity['visible_to_customer'] = 1;
                            $dataactivity['project_id']          = $id;
                            $dataactivity['dateadded']           = date('Y-m-d H:i:s');
...
public function updatedeal_byapi($data, $id)
{
    ...
    $original_project = $this->getproject($id);
    ...
    if(count($_pm) > 0){
        if ($this->add_edit_members_byapi($_pm, $id)) {
            $affectedRows++;
        }
    }
    if (isset($data['mark_all_tasks_as_completed'])) {
...
public function check_deal_exist($data) {
    $id = $data['id'];
    $dealname = $data['name'];

    $this->db->select('*');
    $this->db->where('id !=', $id);
    $this->db->where('name', $dealname);
...
public function reverData($id) {
    //Get Deal Id
    
    $this->db->where('imported_id', $id);
    $result = $this->db->get(db_prefix() . 'projects')->result_array();
    //pre($result);
    foreach($result as $data) {
        //pre($data);
        $dealId = $data['id'];
        //Delete Deals Contact Person
        $this->db->where('project_id', $dealId);
        $this->db->delete('tblproject_contacts');

        $this->db->where('project_id', $dealId);
        $this->db->delete('tblproject_members');
    }
    ...
    //Delete Activities
    $this->db->where('imported_id', $id);
    $this->db->delete('tbltasks');

    $update['status'] = 'Reverted';
    $this->db->where('import_id', $id);
    $this->db->update(db_prefix() . 'importfiles', $update);

    return $id;
}

public function get_uploaded_files() {
    $select = '*,(select firstname from tblstaff where staffid = import_by) as name';
    $this->db->select($select);
    $result = $this->db->get(db_prefix() . 'importfiles')->result_array();
    if($result) {
        return $result;
    } else {
        return false;
...
public function get_primary_contact($id){
	$table1 = db_prefix() . 'project_contacts pc';
	$table2 = db_prefix() . 'contacts c';
	$this->db->select('c.*');
	$this->db->from($table1);
	$join_cond = 'c.id = pc.contacts_id';
	$condition = array('pc.project_id'=>$id,'pc.is_primary'=>1);
	$this->db->join($table2,$join_cond);
	$this->db->where($condition);
    $result = $this->db->get()->row();
	return $result;
}

public function get_activity_status($id) {
    $this->db->where('rel_id', $id);
    $this->db->where('rel_type', 'project');
    $result = $this->db->get(db_prefix() . 'tasks')->result_array();   
    if($result) {
        return $result;
    } else {
        return false;
...
public function storage_message($id){
...
public function reply_messages($id){
	$table = db_prefix() . 'reply';
	$this->db->select('*');
	$this->db->from($table);
	$condition = array('local_id'=>$id);
	$this->db->where($condition);
    $result = $this->db->get()->result_array();
	return $result;
}
...
public function all_activiites()
{
	...
	public function get_user_manger($manger_id)
    {
        $this->db->where('active', 1);
        $this->db->where('reporting_to', $manger_id);
        $this->db->where('action_for', 'Active');
        $this->db->order_by('firstname','asc');
        $emp = $this->db->get('staff')->result_array();
        return $emp;
    }
	public function get_all_targets($status)
    {
        $this->db->where('target_status', $status);
        $emp = $this->db->get('target')->result_array();
        return $emp;
    }
	public function get_target_id($target_id)
    {
        $this->db->where('id', $target_id);
        $emp = $this->db->get('target')->result_array();
        return $emp;
    }
	public function get_all_deal_target($target_id)
    {
        $this->db->where('target_id', $target_id);
		$this->db->select(array('pipeline'));
        $deals = $this->db->get('target_pipeline')->result_array();
        return $deals;
    }
	public function get_all_activity_target($target_id)
    {
        $this->db->where('target_id', $target_id);
		$this->db->select(array('pipeline'));
        $deals = $this->db->get('target_pipeline')->result_array();
	...
	public function get_all_interval($target_id)
    {
        $this->db->where('target_id', $target_id);
		if(get_option('target_company') == 'Calendar'){
			$this->db->order_by('calender','asc');
		}
		else{
			$this->db->order_by('finance','asc');
		}
        $intervals = $this->db->get('target_interval')->result_array();
        return $intervals;
    }
	public function get_target_user($target_id){
		$this->db->where('target_id', $target_id);
        $users = $this->db->get('target_user')->result_array();
        return $users;
	}
	public function get_target_manager($target_id){
		$this->db->where('target_id', $target_id);
        $users = $this->db->get('target_manager')->result_array();
        return $users;
	}
	public function get_pipe_status(){
		$this->db->where('status', 0);
		$this->db->order_by('statusorder','asc');
		 $pipelines = $this->db->get(db_prefix() . 'projects_status')->result_array();
		 return $pipelines;
	}
	public function get_pipe_statusbyid($id){
		$this->db->where('id', $id);
		$this->db->where('status', 0);
		$this->db->order_by('statusorder','asc');
		 $pipelines = $this->db->get(db_prefix() . 'projects_status')->result_array();
	...
	public function get_pipe_value($pipe_stages){
		...
		if(!empty($need_fields) && in_array("status", $need_fields)){ 
			if(!empty($pipe_stages)){
				foreach($pipe_stages as $pipe_stage1){
					$cond_like = array('status'=>$pipe_stage1);
					$this->db->like($cond_like);
				}
			}
		}
		else{
	...
	public function get_all_pipe_stages($target_id)
    {
        $this->db->where('target_id', $target_id);
		$this->db->select(array('stage_id'));
        $deals = $this->db->get('target_stage')->result_array();
        return $deals;
    }
	public function get_edit_pipe_stages($target_id,$stage_id)
    {
        $this->db->where('target_id', $target_id);
	...
	public function update_multiple($data, $ids)
    {
		...
		if (isset($data['tags'])) {
			foreach($req_ids as $id){
				if(!empty($id)){
					handle_tags_save($data['tags'], $id, 'project');
				}
			}
			unset($data['tags']);
		}
		...
		if (isset($data['project_start_date'])) {
			$data['start_date'] = $data['project_start_date'];
			unset($data['project_start_date']);
		}
		if (isset($data['project_deadline'])) {
			$data['deadline'] = $data['project_deadline'];
			unset($data['project_deadline']);
		}
		if(!empty($data)){
			$conditionArray = array($ids);
			$this->db->where_in("id", $target_ids);
			$this->db->update("projects", $data);
		}
		return true;
	}
	public function update_multiple_custom($data, $ids,$upd_id){
		$req_ids = rtrim($ids,",");
		...
		if(!empty($target_ids)){
			foreach($target_ids as $target12){
				...
				if(empty($qyt)){
					...
					$ins_data['relid']  = $target12;
					$ins_data['fieldto']  = 'projects';
					$ins_data['fieldid']  = $upd_id;
					$this->db->insert('customfieldsvalues',$ins_data);
				}
			}
		}
		$result = "update tblcustomfieldsvalues SET value ='".$data['value']."' where relid in(".$req_ids.") and fieldto = 'projects' and fieldid = '".$upd_id."' ";
		$qyt = $this->db->query($result);
		return true;
	}
	public function get_multiples($target_id)
    {
		$target_ids = explode(',',$target_id);
        $this->db->where_in('id', $target_ids);
        $this->db->where('start_date!=', '0000-00-00');
		$this->db->select(array("start_date"));
        $deals = $this->db->get('projects')->result_array();
        return $deals;
    }
	
	public function get_pipelinestages($target_id)
    {
		$target_ids = explode(',',$target_id);
        $this->db->where_in('id', $target_ids);
        $this->db->where('pipeline_id!=', '0');
        $this->db->where('pipeline_id!=', NULL);
		$this->db->select(array("pipeline_id"));
        $deals = $this->db->get('projects')->result_array();
		$deals = array_column($deals,'pipeline_id');
		$this->db->where_in('id', $deals);
		$this->db->where('publishstatus', 1);
		$this->db->select(array("status"));
        $all_pipelines = $this->db->get('pipeline')->result_array();
		$all_pipelines = array_column($all_pipelines,'status');
		$target_values = $stages = array();
		if(!empty($all_pipelines)){
			foreach($all_pipelines as $all_pipeline1){
				$pipe_ids = explode(',',$all_pipeline1);
				if(!empty($pipe_ids)){
					foreach($pipe_ids as $pipe_id1){
						if(!in_array($pipe_id1,$stages)){
							$stages[] = $pipe_id1;
						}
					}
				}
		...
		if(!empty($stages)){
			$this->db->where_in('id', $stages);
		}
        $all_stages = $this->db->get('tblprojects_status')->result_array();
        return $all_stages;
    }
	public function pipelinestagebypipelineid($pipelineid){
	...
	public function get_primary_project_contact($id)
    {
        $this->db->select('*');
        $this->db->where('project_id', $id);
        $this->db->where('is_primary', 1);
        return $this->db->get(db_prefix() . 'project_contacts')->row();
    }
	public function get_org_contact($customer_id = '', $where = ['active' => 1])
    {
		if(!is_admin(get_staff_user_id())) {
            $this->db->where('('.db_prefix().'contacts.addedfrom="'.get_staff_user_id().'" OR ('.db_prefix().'contacts.userid IN (select userid from tblclients where '.db_prefix().'clients.addedfrom="'.get_staff_user_id().'")) OR tblcontacts.id IN (select contacts_id from tblproject_contacts where project_id IN (select project_id from tblproject_members where staff_id = "'.get_staff_user_id().'") ) OR tblcontacts.id IN (select contacts_id from tblproject_contacts where project_id IN (select tblprojects.id from tblprojects where tblprojects.teamleader = "'.get_staff_user_id().'") ) ) ');
        }
        $this->db->where($where);
        if ($customer_id != '') {
	...
	public function get_contactsbyids($ids){

		$req_ids = explode(',',$ids);
		$this->db->where_in('id', $req_ids);
		$this->db->where('active',1);
		$this->db->where('deleted_status',0);
		$this->db->select('*');
		return $this->db->get(db_prefix() . 'contacts')->result_array();
	}
	public function edit_desc_project($desc,$project_id){
