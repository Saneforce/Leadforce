...
class Staff_model extends App_Model
{
    ...
    public function get_my_hierarchy_staffids($id = 0)
    {
        $ids = array();
        $id = $this->get(get_staff_user_id());
        if(isset($id->role) && $id->role > 1){
            $ids[] = $id->staffid;
            $this->db->where('reporting_to',  $id->staffid);
            $staff = $this->db->select('staffid')->get(db_prefix() . 'staff')->result();
    ...
    public function get_my_staffids($id = 0)
    {
        $ids = array();
        $id = $this->get(get_staff_user_id());
        if(isset($id->role) && $id->role > 1){
            ...
            $this->db->where('emp_id',  get_staff_user_id());
    ...
    {
        $pmids = array();
        if(isset($where['pipeline_id'])){
            if(($where['pipeline_id']) > 0){
            $pmidsq = $this->db->where('id', $where['pipeline_id'])->get(db_prefix() . 'pipeline')->row()->teammembers;
            $pmids = explode(',',$pmidsq );
            }
            unset($where['pipeline_id']);
        }
...
//echo $this->db->last_query(); exit;
    ...
    {
        $select_str = '*,CONCAT(firstname," ",lastname) as full_name, (select name from tbldesignations where designationid = designation) as desig';
        $this->db->select($select_str);
        $this->db->order_by('reporting_to', 'asc');
        $result = $this->db->get(db_prefix() . 'staff')->result_array();
        $hierarchyTree = array();
        foreach ($result as $val) {
            $hierarchyTree[$val['staffid']] = $val;
        }
        return $hierarchyTree;
    }

    function getOwnerHierarchy($id) {
        ...
        if($result) {
           // pre($result);
            foreach ($result as $val) {
                $hierarchyTree[$val['staffid']] = $val;
                $this->getOwnerHierarchy($val['reporting_to']);
            }
        } else {
            return $hierarchyTree;
        }
    ...
    protected function getHierarchyTree($level = 0, $prefix = '') {
        $rows = $this->db
            ->select('*')
            ->where('reporting_to', $level)
        ...
        if (count($rows) > 0) {
            foreach ($rows as $row) {
                $category .= $row->staffid . ",";
                // Append subcategories
                $category .= $this->getHierarchyTree($row->staffid, $prefix . '-');
            }
        }
        return $category;
    }
    ...
    public function getFollowersViewList() {
        $ids = array();
        $staffids = rtrim($this->getHierarchyTree(get_staff_user_id()), ',');
        $hie_ids = explode(',',$staffids);
        ...
        if(count((array)$followers) > 0){
            if($followers->p_type == 1) {
                $this->db->where('emp_id',  get_staff_user_id());
                $follow = $this->db->select('follower_id, permission')->get(db_prefix() . 'followers')->result();
                foreach($follow as $sk => $sv){
                    if(!in_array($sv->follower_id,$hie_ids)) {
                        if($sv->permission == 'view') {
                            $ids[] = $sv->follower_id;
                        }
                    }
                }
            } elseif($followers->p_type == 2) {
                $rows = $this->db
                        ->select('*')
                        ->where('role != ', 1)
                        ->where('action_for', 'Active')
                        ...
                        ->result();
                foreach($rows as $sk => $sv){
                    if(!in_array($sv->staffid,$hie_ids) && $sv->staffid != get_staff_user_id()) {
                        $ids[] = $sv->staffid;
                    }
                }
            } else {
    ...
    protected function getCategoryTree($level = 0, $prefix = '') {
        ...
        if (count($rows) > 0) {
            foreach ($rows as $row) {
                $category .= $row->staffid . ",";
                // Append subcategories
                $category .= $this->getCategoryTree($row->reporting_to, $prefix . '-');
    ...
    public function printCategoryTree($id) {
        $staffids = rtrim($this->getCategoryTree($id), ',');
    ...
    public function add($data)
    {
        ...
        if(!isset($data['role']) && isset($data['designation'])){
            ...
            $data['role'] = $this->designation_model->get_designation_roles($data['designation']);
        }
    ...
    public function update($data, $id)
    {
        ...
        if (isset($data['permissions'])) {
            ...
            $roleExist = $this->db->get(db_prefix() . 'staff')->row();
            if($roleExist) {
                $permissions = $data['permissions'];
            } else {
                $this->db->where('roleid', $data['role']);
...
//echo "111";
  ...
  //pre($permissions);
     ...
     * @return mixed if id is passed return object else array
     */
    public function getstaff($id = '', $where = [], $leaderid = '', $pipeline = '')
    {
        ...
        if (($key = array_search($leaderid, $staffids)) !== FALSE) {
            unset($staffids[$key]);
        }
        $this->db->where('active', '1');
        $this->db->where('staffid !=', $leaderid);
    ...
    public function getstaffdetails($id = '', $where = [])
    {
        $pmids = array();
        if(isset($where['pipeline_id'])){
            if(($where['pipeline_id']) > 0){
