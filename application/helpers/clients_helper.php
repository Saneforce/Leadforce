...
function get_product_profile_tabs()
{
    return get_instance()->app_tabs->get_product_profile_tabs();
}
 ...
 * @return null
 */
function app_init_product_profile_tabs()
{
    $CI = &get_instance();
    $CI->app_tabs->add_product_profile_tab('profile', [
        'name'     => _l('client_add_edit_profile'),
        'icon'     => 'fa fa-user-circle',
        'view'     => 'admin/products/groups/profile',
        'position' => 5,
    ]);

    $CI->app_tabs->add_product_profile_tab('notes', [
        'name'     => _l('contracts_notes_tab'),
        'icon'     => 'fa fa-sticky-note-o',
        'view'     => 'admin/products/groups/notes',
        'position' => 15,
    ]);
    
    $CI->app_tabs->add_product_profile_tab('projects', [
        'name'     => _l('projects'),
        'icon'     => 'fa fa-bars',
        'view'     => 'admin/products/groups/projects',
        'position' => 65,
    ]);

    $CI->app_tabs->add_product_profile_tab('attachments', [
        'name'     => _l('customer_attachments'),
        'icon'     => 'fa fa-paperclip',
        'view'     => 'admin/products/groups/attachments',
        'position' => 80,
    ]);
...
function get_product_attachments($id) {
    $CI = &get_instance();
    $CI->db->where('productid', $id);
    $product = $CI->db->get(db_prefix() . 'product_files')->result_array();
    $attachments = [];
    $attachments['files'] = $product;
    return hooks()->apply_filters('product_attachments', $attachments, $id);
}
