...
function get_available_staff_permissions($data = [])
{
    ...
    $corePermissions = [
		...
		'tasktype' => [
            ...
            'capabilities' => $withNotApplicableViewOwn,
             'help'        => [
                'view'     => _l('help_tasktype_permissions'),
                'view_own' => _l('permission_tasktype_based_on_assignee'),
            ],
        ],
        'checklist_templates' => [
    ...
    // modules permissions not active
    $mp_not_active = get_instance()->roles_model->get_modules_permissions_not_active();
    foreach ($mp_not_active as $key => $value) {
       if(isset($corePermissions[$value])){
           unset($corePermissions[$value]);
       }
    }
...
function get_invoice_pdf_config()
{
    $CI = & get_instance();
	
    $CI->db->where('id', 1);
    $result = $CI->db->select('*')->from(db_prefix() . 'invoice_pdf_config')->get()->row();
