...
function handle_lead_attachments($leadid, $index_name = 'file', $form_activity = false)
{
    ...
    if (isset($_FILES[$index_name]['name']) && $_FILES[$index_name]['name'] != '' && !empty($_FILES[$index_name]['name'])) {
        hooks()->do_action('before_upload_lead_attachment', $leadid);
        $count=0;
        foreach ($_FILES[$index_name]['name'] as $filename) 
        {
            $path = get_upload_path_by_type('lead') . $leadid . '/';
            // Get the temp file path
            $tmpFilePath = $_FILES[$index_name]['tmp_name'][$count];
            // Make sure we have a filepath
            if (!empty($tmpFilePath) && $tmpFilePath != '') {
                ...
                $filename    = unique_filename($path, $_FILES[$index_name]['name'][$count]);
                $newFilePath = $path . $filename;
                // Upload the file into the company uploads dir
                if (move_uploaded_file($tmpFilePath, $newFilePath)) {
                    $CI = & get_instance();
                    ...
                    $data[] = [
                        'file_name' => $filename,
                        'filetype'  => $_FILES[$index_name]['type'][$count],
                        ];
                    $CI->leads_model->add_attachment_to_database($leadid, $data, false, $form_activity);
                }
            }
...
function handle_product_attachments_upload($id, $customer_upload = false)
{
    $path          = get_upload_path_by_type('files') . $id . '/';
    $CI            = & get_instance();
    $totalUploaded = 0;
        ...
        for ($i = 0; $i < count($_FILES['file']['name']); $i++) {
            ...
            if (!empty($tmpFilePath) && $tmpFilePath != '') {
                ...
                if (move_uploaded_file($tmpFilePath, $newFilePath)) {
                    ...
                    $CI->misc_model->add_product_attachment_to_database($id, 'product', $attachment);
                    $totalUploaded++;
                }
            }
        }
...
function get_upload_path_by_type($type)
{
    ...
    switch ($type) {
        ...
        break;
        case 'files':
            $path = PRODUCT_ATTACHMENTS_FOLDER;
