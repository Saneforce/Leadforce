 ...
 * @return array
 */
function data_tables_init($aColumns, $sIndexColumn, $sTable, $join = [], $where = [], $additionalSelect = [], $sGroupBy = '', $searchAs = [], $taskpage = '', $wherewo = [])
{
    $CI          = & get_instance();
    ...
    if ($CI->input->post('order')) {
		...
		if($sTable == 'tbltasks' && $columnName == '') {
			$columnName = ' tbltasks.startdate  ';
		}
            ...
            if (stripos($columnName, 'AVG(') !== false || stripos($columnName, 'SUM(') !== false) {
                ...
                if (($__post['columns'][$i]) && $__post['columns'][$i]['searchable'] == 'true') {
                    ...
                    if ($useMatchForCustomFieldsTableSearch === 'true' && startsWith($columnName, 'ctable_')) {
						...
						if($sTable == 'tbltarget ' || $sTable == 'tbltarget') {
							if (strpos($columnName, 'GROUP_CONCAT') === false) {
								$req_col = explode(' ',$columnName);
								$columnName = $req_col[0];
							}
							if (strpos($columnName, 'as') !== false) {
								$req_col = explode(' as',$columnName);
								$columnName = $req_col[0];
							}

							$sWhere .=  $columnName . " LIKE '%" . $search_value . "%' OR ";
						}else{
    ...
    if($sTable == 'tblclients' && $__post['order'][0]['column'] == 0 && count($__post['order']) == 1) {
        $sOrder = ' ORDER BY tblclients.company ASC ';
    }
    
    if($sTable == 'tblprojects' && !isset($__post['order'])) {
        $sOrder = ' ORDER BY tblprojects.start_date DESC ';
    }

    if($sTable == 'tbltasks' && !isset($__post['order'])) {
        $sOrder = ' ORDER BY tbltasks.startdate DESC ';
    }

    if($sTable == 'tblleads' && !isset($__post['order'])) {
        $sOrder = ' ORDER BY tblleads.id DESC ';
    }
	...
	if($sTable == 'tbltarget ' || $sTable == 'tbltarget') {
		...
		";
			...
			$_additionalSelect = ltrim($_additionalSelect,',');
			$sQuery = '
		SELECT SQL_CALC_FOUND_ROWS ' . str_replace(' , ', ' ', implode(', ', $_aColumns)) . ' ' . $_additionalSelect . "
		FROM $sTable
		" . $join . "
    ...
    $wherewo = implode(' ', $wherewo);
    if ($sWhere == '') {
        $wherewo = trim($wherewo);
        if (startsWith($wherewo, 'AND') || startsWith($wherewo, 'OR')) {
            if (startsWith($wherewo, 'OR')) {
                $wherewo = substr($wherewo, 2);
            } else {
                $wherewo = substr($wherewo, 3);
            }
            $wherewo = 'WHERE ' . $wherewo;
        }
    }
       ...
       $output = [
		...
		if(!empty($tasktypes)){
			...
			if (count($_tasktypes) > 0) {
				$req_task_type = ' AND (' . db_prefix() . 'tasks.tasktype IN (' . implode(', ', $_tasktypes) . ')) ';
			}
		}
        ...
        $taskQry = '
            ...
            " . $join . "
            $sWhere
            " . $wherewo . " WHERE tblprojects.id = ".$exp_page[1].$call.$req_task_type."  
            $sGroupBy
            ";
        $statusCounts = datatable_tasks_summary_data($taskQry);
        //pre($statusCounts);
		...
		if($sTable == db_prefix().'customfields') {
		   ...
		   $CI->db->where('fieldto','Person');
			$CI->db->update($sTable, ['fieldto' => 'contacts']);
		}
        $output = [
            'draw'                 => $__post['draw'] ? intval($__post['draw']) : 0,
            'iTotalRecords'        => $iTotal,
...
function datatable_tasks_summary_data($sql)
{
    $CI          = & get_instance();
    $tasks_summary = [];
    // $this->load->model('tasks_model');
    // $statuses      = array('id' => array('1','2','3','4','5'));
    //$this->load->model('tasks_model');
    $statuses      = $CI->tasks_model->get_statuses();
    //pr($statuses); exit;
    foreach ($statuses as $status) {
        $tasks_where = 'tbltasks.rel_type != "" AND ';
        if($status['id'] == 1) {
            //$tasks_where = $tasks_where.' AND startdate > ' . date('Y-m-d');
            $tasks_where = ' date(tbltasks.startdate) > CURDATE() AND tbltasks.status != 5 ';
        } elseif($status['id'] == 3) {
            //$tasks_where = $tasks_where.' AND startdate = ' . date('Y-m-d');
            $tasks_where = ' date(tbltasks.startdate) = CURDATE() AND tbltasks.status != 5 ';
        } elseif($status['id'] == 2) {
            ...
            $tasks_where = 'tbltasks.status = ' . $status['id'];
        }
        $sdate = date('Y-m-d', strtotime($aRow['startdate'])); 
        if(strtotime($sdate) == strtotime(date('Y-m-d')) && $aRow['status'] != 5) {
            $tasks_where = 'tbltasks.status = 3';
        }
        if(strtotime($sdate) > strtotime(date('Y-m-d')) && $aRow['status'] != 5) {
            $tasks_where = 'tbltasks.status = 1';
        }
        
        $sqlQry = $sql.' AND '.$tasks_where;
        
        //$mysqlQry = $sql.$tasks_my_where;
        
        $_query = $CI->db->query($sqlQry)->result_array();
        
//echo $tasks_where; exit;
       //pre($_query);
        $summary                   = [];
        $summary['total_tasks']    = count($_query);
...
function render_datatable($headings = [], $class = '', $additional_classes = [''], $table_attributes = [])
{
    ...
    foreach ($headings as $heading) {
        if (!is_array($heading)) {
            ...
            if($class == 'leads') {
                if($i == 0) {
                    $table .= '<th class="sorting_disabled not-export" rowspan="1" colspan="1" aria-label=" - "><span class="hide"> - </span><div class="checkbox mass_select_all_wrap"><input type="checkbox" id="mass_select_all" data-to-table="leads"><label></label></div></th>';
                } else {
                    $th_attrs = '';
