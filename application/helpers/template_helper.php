...
function app_js_alerts()
{
    ...
    echo '</script>';
	$CI->session->unset_userdata('message-success');
	$CI->session->unset_userdata('message-warning');
	$CI->session->unset_userdata('message-info');
	$CI->session->unset_userdata('message-danger');
}
...
function get_company_logo($uri = '', $href_class = '', $type = '')
{
    ...
    if ($uri == '') {
        $logoURL = site_url();
        $logoURL = $logoURL.'/admin/authentication';
    } else {
        $logoURL = site_url($uri);
        $logoURL = $logoURL.'/authentication';
    }
    ...
    if ($company_logo != '') {
        ...
        <img src="' . base_url('uploads/company/' . $company_logo) . '?v=1.0" class="img-responsive" alt="' . html_escape($company_name) . '">
        </a>';
    } elseif ($company_name != '') {
