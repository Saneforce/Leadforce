...
class App_mail_template
{
    ...
    public function prepare($email = null, $template = null, $params = [])
    {
        ...
        if($_POST['tempid']) {
            if($slug == 'invoice-already-send')
                $template = $this->ci->emails_model->get(['language' => $language, 'slug' => 'invoice-send-to-client', 'emailtemplateid' => $_POST['tempid']], 'row');
            else
                $template = $this->ci->emails_model->get(['language' => $language, 'slug' => $slug, 'emailtemplateid' => $_POST['tempid']], 'row');
        } else {
        ...
        if (!$template) {
            if($_POST['tempid']) {
                if($slug == 'invoice-already-send')
                    ...
                    $template = $this->ci->emails_model->get(['language' => $language, 'slug' => $slug, 'emailtemplateid' => $_POST['tempid']], 'row');
                //$template = $this->ci->emails_model->get(['language' => 'english', 'slug' => $slug, 'emailtemplateid' => $_POST['tempid']], 'row');
            } else {
