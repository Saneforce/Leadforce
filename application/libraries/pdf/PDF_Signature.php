...
trait PDF_Signature
{
    public function process_signature()
    {
        ...
        || ($this->type() == 'credit_note') && get_option('show_pdf_signature_credit_note') == 1) {
            ...
            $this->ln(13);
            if (isset($GLOBALS['invoice_pdf']) && !empty($GLOBALS['invoice_pdf'] && $invoicepdf->use_as_default == 1)) {
                
                $blankSignatureLine .= '<br /><br /><img src="' . site_url($invoicepdf->signature) . '" />';
        ...
        if (isset($GLOBALS['estimate_pdf']) && !empty($GLOBALS['estimate_pdf']->signature)) {
            ...
            $customerSignaturePath = get_upload_path_by_type('proposal') . $proposal->id . $dbname[0] . '/' . $proposal->signature;
