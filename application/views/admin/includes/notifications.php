  ...
  foreach($_notifications as $notification){ ?>
    <li class="relative notification-wrapper" data-notification-id="<?php echo $notification['id']; ?>">
      <?php if(!empty($notification['link'])){ 
        $taskid = '';
        if (strpos($notification['link'], '#taskid=') !== false) {
            $exp = explode('#taskid=',$notification['link']);
            $taskid = 'onclick="edit_task('.$exp[1].');';
        }
        ?>
        <a href="<?php echo admin_url($notification['link']); ?>" <?php echo $taskid; ?> return false;" class="notification-top notification-link 123456">
        <?php } ?>
