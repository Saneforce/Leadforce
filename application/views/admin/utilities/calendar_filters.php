...
<div id="calendar_filters" style="<?php if(!$this->input->post('calendar_filters')){ 
//echo 'display:none;';
 } ?>">
    <?php echo form_open(); ?>
						...
						<div class="col-md-4">
							...
							<option value=""><?php echo _l('all'); ?></option>
								<?php foreach($pipelines as $status){
									?>
									<option value="<?php echo $status['id']; ?>"<?php if($dash_form_data['pipelines'] == $status['id']){echo ' selected';} ?>>
										<?php echo $status['name']; ?>
									</option>
								<?php } ?>
							</select>
								<?php /* <select data-actions-box="true" name="status[]" class="selectpicker" data-width="100%" multiple="true">
									<?php foreach($statuses as $status) { ?>
										<?php if(in_array($status['id'], $dash_form_data['status'])) { ?>
                          ...
                          <?php $selectedMember = $dash_form_data['member']; ?>
							<div class="col-md-5"> 
                            ...
                            <?php if(is_admin(get_staff_user_id()) || count($project_members) > 1) { ?> 
                                    ...
                                    <?php foreach($project_members as $member) { ?>
                                        <?php if(!is_admin(get_staff_user_id()) && count($project_members) == 1) { ?> 
                                            <option value="<?php echo $member['staff_id']; ?>" selected >
