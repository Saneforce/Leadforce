      ...
      <div class="panel_s">
       ...
       <hr class="hr-panel-heading" />
          ...
          <div class="profile mtop20 display-inline-block">
            <h4>
			<?php 
			/*$ch_prof_name = $staff_p->firstname . ' ' . $staff_p->lastname;
			
			$ch_prof_count = strlen($ch_prof_name);
			if($ch_prof_count<=21){
				echo $ch_prof_name;
			}else{
				echo substr($ch_prof_name,0,21).'...';
			}*/
			?>
     ...
     if(page <= total_pages){
      $.post(admin_url + 'staff/notifications',{page:page}).done(function(response){
       ...
       $.each(response,function(i,obj){
        ...
        if(obj.link){
         var taskid = obj.link.split('#taskid=');
         link_notification= 'onclick="edit_task('+taskid[1]+'); return false;" data-link="'+admin_url+obj.link+'"';
