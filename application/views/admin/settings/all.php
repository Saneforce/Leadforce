   ...
   <div class="col-md-3">
    <ul class="nav navbar-pills navbar-pills-flat nav-tabs nav-stacked">
      ...
      foreach ($tabs as $group) {
        ...
        if ($group['slug'] != 'sms') {
          if($group['slug'] == 'enable_call' || $group['slug'] == 'agent') {
          ?>
        <li
        <?php if ($tab['slug'] == $group['slug']) {
              echo " class='active'";
          } ?>>
        <a href="<?php echo admin_url('call_settings/' . $group['slug']); ?>" data-group="<?php echo $group['slug']; ?>">
          <?php echo $group['name']; ?></a>
        </li>
          <?php }else if($group['slug'] =='mandatory_deal' || $group['slug'] =='mandatory_activity'){?>
			<li
        <?php if ($tab['slug'] == $group['slug']) {
              echo " class='active'";
          } ?>>
        <a href="<?php echo admin_url('mandatory_fields/' . $group['slug']); ?>" data-group="<?php echo $group['slug']; ?>">
          <?php echo $group['name']; ?></a>
        </li>
		  <?php 
		  } else { ?>
...
<script>
  ...
  <?php if(isset($tab['slug']) && $tab['slug'] == 'email_local'){?>

<script>
$("#settings-form").submit(function() {
	var email_local = $('#email_local').val();    
	if(email_local == 'no'){
		if( !confirm('Email will not store in local.') ) 
