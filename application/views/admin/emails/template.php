...
<script>
   $(function(){
	   <?php if($template->type!='activity_show_alert' && $template->type!='proposal_show_alert' && $template->type!='target_show_alert'){?>
     ...
     $('.add_merge_field').on('click', function(e) {
      e.preventDefault();
	   ...
	   <?php }else{?>
	   var req_val = $('.tinymce-manual').val();
	   var ch_val  = req_val + $(this).text();
	    $('.tinymce-manual').val(ch_val);
	   <?php }?>
    });
