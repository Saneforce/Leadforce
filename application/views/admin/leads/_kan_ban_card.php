	...
	<div class="panel-body lead-body">
		<div class="row">
			<div class="col-md-12 lead-name">
				<?php if ($lead['assigned'] != 0) { ?>
					<a href="<?php echo admin_url('profile/' . $lead['assigned']); ?>" data-placement="right" data-toggle="tooltip" title="<?php echo get_staff_full_name($lead['assigned']); ?>" class="pull-left mtop8 mright5">
						<?php echo staff_profile_image($lead['assigned'], array('staff-profile-image-xs')); ?>
					</a>
				<?php  } ?>
				<a href="<?php echo admin_url('leads/index/'.$lead['id']); ?>" onclick="init_lead(<?php echo $lead['id']; ?>);return false;" class="pull-left">
					<span class="inline-block mtop10 mbot10">#<?php echo $lead['lead_name']; ?></span>
				</a>
			</div>
			<?php if($this->session->has_userdata('leads_kanban_view') && $this->session->userdata('leads_kanban_view') == 'true') { ?>
				<div class="col-md-6 text-muted">
					<small  class="text-dark"><?php echo _l('teamleader').': '.get_staff_full_name($lead['teamleader']); ?></small>
				</div>
				<div class="col-md-6 text-right text-muted">
					<?php if(is_date($lead['lastcontact']) && $lead['lastcontact'] != '0000-00-00 00:00:00'){ ?>
						<small class="text-dark"><?php echo _l('leads_dt_last_contact'); ?>
							<span class="bold">
					...
					<?php } ?>
					<small class="text-dark"><?php echo _l('lead_created'); ?>: 
						<span class="bold">
			...
			<div id="kan-ban-expand-<?php echo $lead['id']; ?>" class="padding-10" style="display:none;">
				...
				<p class="text-muted lead-field-heading"><?php echo _l('task_assigned'); ?></p>
				<p class="bold font-medium-xs"><?php echo ($lead['assigned'] != '' ? get_staff_full_name($lead['assigned']) : '-') ?></p>
			</div>
		</div>
   </div>
</li>
