			...
			<div class="modal-body">
				<div class="row">
					<div class="col-md-12">
						...
						<div class="col-md-12"><h3>Conversion Rate</h3></div>
						<a href="javascript:void(0);" class="addfield_button" title="Add field" style="position:relative; top:10px;height:40px; left:15px; clear:both; float:left;"><i class="fa fa-plus"></i> Add More</a>
							<div id="crate"></div>
						</div>
					</div>
				</div>
...
<script>
	$(function(){
		...
		$('#currency_modal').on('show.bs.modal', function(event) {
			...
			if (typeof(id) !== 'undefined') {
				...
				var url =  admin_url+'currencies/getconvertion_rate';
				$.ajax({
					type: "POST",
					url: url,
					data: {name:name},
	...
	$('.addfield_button').click(function(){
		var name = $("#name").val();
        var currency= $("select[name=\'currency_to[]\']").map(function() {
            return $(this).val();
        }).toArray();
        //alert(currency);
		var url =  admin_url+'currencies/getaddfields';
		
		$.ajax({
			type: "POST",
			url: url,
			data: {currency:currency,name:name},
			success: function(msg){
				$('#crate').append(msg);
			}
		});
	...
	$("#crate").on('click', '.remove_button', function(e){
        e.preventDefault();
