...
<div class="modal-dialog" role="document">
   <div class="modal-content">
            ...
            <div class="col-md-12">
			  ...
			  ?>
               ...
               // onclick and onfocus used for convert ticket to task too
                //echo render_textarea('description','',(isset($task) ? $task->description : ''),array('rows'=>4,'placeholder'=>_l('task_add_description'),'data-task-ae-editor'=>true, !is_mobile() ? 'onclick' : 'onfocus'=>(!isset($task) || isset($task) && $task->description == '' ? 'init_editor(\'.tinymce-task\', {height:50, auto_focus: true, menubar: false});' : '')),array(),'no-mbot','tinymce-task'); ?>
                <!-- <p></p> -->
			   ...
			   <?php if(!empty($reminder_settings->customer_reminder) && (  str_contains($reminder_settings->reminder_type, 'customer')  && $reminder_settings->customer_reminder=='required_activities')){?>
      ...
      <?php 
            ...
            if($assigneduser && $btn == 'btn-disable') {
      ...
      <?php if($task->status != Tasks_model::STATUS_COMPLETE && ($task->current_user_is_assigned || has_permission('tasks','','edit') || $task->current_user_is_creator)){ ?>
        ...
        <p class="no-margin pull-left" style="<?php echo 'margin-'.(is_rtl() ? 'left' : 'right').':5px !important'; ?>">
            <a href="#" class="btn btn-default pull-left mright5 <?php echo $btn; ?>" id="task-single-mark-complete-btn" autocomplete="off" data-loading-text="<?php echo _l('wait_text'); ?>" onclick="mark_complete(<?php echo $task->id; ?>); return false;" data-toggle="tooltip" title="<?php echo _l('task_single_mark_as_complete'); ?>">
            &nbsp; <?php echo _l('task_single_mark_as_complete'); ?>
            </a>
         </p>
        <?php } ?>
         ...
         <p class="no-margin pull-left" style="<?php echo 'margin-'.(is_rtl() ? 'left' : 'right').':5px !important'; ?>">
            <a href="#" class="btn btn-info  pull-left mright5 btnmark  <?php echo $btn; ?>" id="task-single-unmark-complete-btn" autocomplete="off" data-loading-text="<?php echo _l('wait_text'); ?>" onclick="unmark_complete(<?php echo $task->id; ?>); return false;" data-toggle="tooltip" title="<?php echo _l('task_unmark_as_complete'); ?>">
            <i class="fa fa-check"></i> <?php echo _l('task_unmark_as_complete'); ?>
            </a>
         </p>
         <?php } ?>
         <?php if(isset($task)){ ?>
               <div class="pull-left mbot10 task-single-menu task-menu-options">
                  <div class="content-menu hide">
                     <ul>
                        <?php if(has_permission('tasks','','create')){ ?>
                        <?php
                  ...
                  <?php if(has_permission('tasks','','delete') || has_permission('tasks','','create')){ ?>
                  <a href="#" onclick="return false;" class="trigger manual-popover mright5 pull-left trible <?php echo $btn; ?>">
                  <i class="fa fa-circle-thin" aria-hidden="true"></i>
                  <i class="fa fa-circle-thin" aria-hidden="true"></i>
                  <i class="fa fa-circle-thin" aria-hidden="true"></i>
                  </a>
         ...
         <!-- <p class="no-margin pull-left" >
            <a href="<?php echo admin_url('tasks/delete_task/'.$task->id); ?>" class="btn btn-danger" title="<?php echo _l('task_single_delete'); ?>" class="_delete task-delete">
