...
<div id="wrapper">
  <div class="content">
          ...
          <?php 
              ...
              if(!is_admin(get_staff_user_id)) {
                   $where_person_qry = 'SELECT  tblcontacts.firstname, tblcontacts.id, tblcontacts.email 
                        ...
                        WHERE  (tblcontacts.id IN (select contacts_id from tblproject_contacts where project_id IN (select project_id from tblproject_members where staff_id = "'.get_staff_user_id().'") ) OR tblcontacts.id IN (select contacts_id from tblproject_contacts where project_id IN (select tblprojects.id from tblprojects where tblprojects.teamleader = "'.get_staff_user_id().'") ) )  AND (tblcontacts.active=1 OR tblcontacts.active=0)  AND tblcontacts.deleted_status=0  AND userids != "" '.$likeqry;
                   $CI          = & get_instance();
                    $list_person     = $CI->db->query($where_person_qry)->result_array();
              }

          ?>
        <div class="inline-block new-contact-wrapper" style="width:100%" data-title="<?php echo _l('customer_contact_person_only_one_allowed'); ?>"<?php if($disable_new_contacts){ ?> data-toggle="tooltip"<?php } ?>>
   ...
   <a href="#" data-toggle="modal" data-target="#contactid_add_modal" return false;" class="btn btn-info new-contact mbot25<?php if($disable_new_contacts){echo ' disabled';} ?>"><?php echo _l('merge_contact'); ?></a>
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        <div class="form-group">
            ...
            ?>  
            <ul id="sortable">
            <?php $projects_list_column_order = (array)json_decode(get_option('contacts_list_column_order')); //pr($projects_list_column_order); ?>
            ...
            <?php } ?>
            <?php foreach($colarr as $ckey=>$cval){ if(!isset($projects_list_column_order[$ckey])){?>
                <li class="ui-state-default"><span class="ui-icon ui-icon-arrowthick-2-n-s"></span>
...
<div class="alpha-filter _filter_data">
                      <ul>
                      <li><a class="btn btn-<?php echo array_search('All', $_SESSION['alpha'])?'info':'default'; ?>" style="padding:10px,15px; margin:0 2px;" href="<?php echo admin_url('all_contacts'); ?>?a=All">All</a></li>
                  <?php foreach(range('A', 'Z') as $letter) {
                    $active = 'default';
                    if (($key = array_search($letter, $_SESSION['alpha'])) !== FALSE) {
                      $active = 'info';
                    }
                      ?>
                  ...
                  <?php if(has_permission('customers','','view') || have_assigned_customers()) {
                     ...
                     if(is_admin(get_staff_user_id)) {
                      ...
                      //pre($_SESSION);
                        ...
                        FROM tblcontacts
                        LEFT JOIN tblclients ON tblclients.userid=tblcontacts.userid LEFT JOIN tblcustomfieldsvalues as ctable_0 ON tblcontacts.id = ctable_0.relid AND ctable_0.fieldto="contacts" AND ctable_0.fieldid=7  where tblcontacts.active=1  AND tblcontacts.deleted_status=0 '.$likeqry;
                        $CI          = & get_instance();
                        ...
                        FROM tblcontacts
                        LEFT JOIN tblclients ON tblclients.userid=tblcontacts.userid LEFT JOIN tblcustomfieldsvalues as ctable_0 ON tblcontacts.id = ctable_0.relid AND ctable_0.fieldto="contacts" AND ctable_0.fieldid=7  where tblcontacts.active=0  AND tblcontacts.deleted_status=0 '.$likeqry;
                        $CI          = & get_instance();
...
//Person
                        if($_GET['contacts'] && $_GET['contacts'] != 'all') {
                            ...
                            WHERE (tblcontacts.id IN (select contacts_id from tblproject_contacts where project_id IN (select project_id from tblproject_contacts where project_id IN (select project_id from tblproject_members where staff_id = "'.get_staff_user_id().'") AND contacts_id = "'.$_GET['contacts'].'" ) ) OR tblcontacts.id IN (select contacts_id from tblproject_contacts where project_id IN (select tblprojects.id from tblprojects where tblprojects.teamleader = "'.get_staff_user_id().'") AND contacts_id = "'.$_GET['contacts'].'" ) )  AND tblcontacts.active=1  AND tblcontacts.deleted_status=0  AND userids != "" '.$likeqry;
                            $CI          = & get_instance();
                            $where_summary_activeperson     = $CI->db->query($where_summary_activeperson_qry)->result_array();
                            ...
                            WHERE  (tblcontacts.id IN (select contacts_id from tblproject_contacts where project_id IN (select project_id from tblproject_contacts where project_id IN (select project_id from tblproject_members where staff_id = "'.get_staff_user_id().'") AND contacts_id = "'.$_GET['contacts'].'" ) ) OR tblcontacts.id IN (select contacts_id from tblproject_contacts where project_id IN (select tblprojects.id from tblprojects where tblprojects.teamleader = "'.get_staff_user_id().'") AND contacts_id = "'.$_GET['contacts'].'" ) )  AND tblcontacts.active=0  AND tblcontacts.deleted_status=0  AND userids != "" '.$likeqry;
                            $CI          = & get_instance();
                            $where_summary_inactiveperson     = $CI->db->query($where_summary_inactiveperson_qry)->result_array();
                        } else {
                            $where_summary_activeperson_qry = 'SELECT  COUNT(*) AS `numrows`
                            ...
                            WHERE  (tblcontacts.id IN (select contacts_id from tblproject_contacts where project_id IN (select project_id from tblproject_members where staff_id = "'.get_staff_user_id().'") ) OR tblcontacts.id IN (select contacts_id from tblproject_contacts where project_id IN (select tblprojects.id from tblprojects where tblprojects.teamleader = "'.get_staff_user_id().'") ) )  AND tblcontacts.active=1  AND tblcontacts.deleted_status=0  AND userids != "" '.$likeqry;
                            $CI          = & get_instance();
                            $where_summary_activeperson     = $CI->db->query($where_summary_activeperson_qry)->result_array();
                            ...
                            WHERE  (tblcontacts.id IN (select contacts_id from tblproject_contacts where project_id IN (select project_id from tblproject_members where staff_id = "'.get_staff_user_id().'") ) OR tblcontacts.id IN (select contacts_id from tblproject_contacts where project_id IN (select tblprojects.id from tblprojects where tblprojects.teamleader = "'.get_staff_user_id().'") ) )  AND tblcontacts.active=0  AND tblcontacts.deleted_status=0  AND userids != "" '.$likeqry;
                            $CI          = & get_instance();
                            $where_summary_inactiveperson     = $CI->db->query($where_summary_inactiveperson_qry)->result_array();
    //TOTAL Organisation
                            $where_summary_totorg_qry = 'SELECT company
...
<script>
 ...
 <?php if(is_gdpr() && get_option('gdpr_enable_consent_for_contacts') == '1'){ ?>
  ...
  <?php } ?>
  _table_api = initDataTable('.table-all-contacts', window.location.href, optionsHeading, optionsHeading, allContactsServerParams, [1,'asc']);
  if(_table_api) {
   <?php if(is_gdpr() && get_option('gdpr_enable_consent_for_contacts') == '1'){ ?>
