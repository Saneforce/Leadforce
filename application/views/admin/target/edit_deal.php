<form action="<?php echo admin_url('target/edit_deal/'.$target_id); ?>" method="post" id="edit_deal1">
	<input type="hidden" name="<?php echo $this->security->get_csrf_token_name();?>" value="<?php echo $this->security->get_csrf_hash();?>">
	<div class="modal-body">
		<div class="form-group">
			<label ><b><?php echo _l('assign');?></b></label>
			<select class="form-control" name="assign" onchange="check_user_edit(this.value)" id="edit_assign">
		...
		<?php if($targets[0]['target_type'] != 'progressed'){?>
			<div class="form-group" id="edit_pipeselect">
				<label > <small class="req text-danger">* </small><b><?php echo _l('pipeline');?></b></label>
				<select class="form-control selectpicker" data-live-search="true" name="select_deal[]"   multiple required data-none-selected-text="<?php echo _l('Select Pipline'); ?>" onchange="sel_deal()" id="select_deal_edit" data-actions-box='true'>
					<?php if(!empty($deals)){
		...
		<?php }else{?>
			<div class="form-group" id="edit_pipe_stage" >
				<label > <small class="req text-danger">* </small><b><?php echo _l('pipeline_stage');?></b></label>
				<select class="form-control selectpicker" data-live-search="true" name="pipeline_stage[]" required  multiple data-none-selected-text="<?php echo _l('Select Pipline Stage'); ?>" id="pipeline_stage_1" data-actions-box='true' onchange="pipeline_stage_edit()">
					<?php if(!empty($pipe_status)){
						foreach($pipe_status as $pipe_status1){
					?>
							<option value="<?php echo $pipe_status1['id'];?>" <?php if(in_array($pipe_status1['id'],$sel_stages)){echo 'selected';}?>><?php echo $pipe_status1['name'];?></option>
					<?php 
						}
					}?>
				</select>
			</div>
			<div class="form-group" id="pipeselect_edit">
				<label > <small class="req text-danger">* </small><b><?php echo _l('pipeline');?></b></label>
				<select class="form-control selectpicker" data-live-search="true" name="select_deal[]" required   data-none-selected-text="<?php echo _l('Select Pipline'); ?>"  id="select_deal_new1" data-actions-box='true'>
					<?php if(!empty($deals)){
						foreach($deals as $deal12){
					?>
							<option value="<?php echo $deal12['id'];?>" <?php if(in_array($deal12['id'],$sel_deals)){echo 'selected';}?>><?php echo $deal12['name'];?></option>
		...
		<div class="form-group">
			...
			<input type="radio" name="tracking_metic" id="track_value" value="Value" onchange="ch_count(this)" style="vertical-align:text-bottom;" <?php if($targets[0]['tracking_metric'] == 'Value'){echo 'checked';}?>>   <?php echo _l('project_cost');?>
		...
		<div class="form-group">
			...
			<div>
				<input type="text" id="start_date_edit" name="start_date" class="form-control datepicker1" style="width:49%;float:left"  value="<?php echo date('d-m-Y',strtotime($targets[0]['start_date']));?>"  required  autocomplete="off"  placeholder="Start Date">
				<input type="hidden" name="goal_val" class="form-control" id="goal_val" >
				<input type="text" name="end_date" id="end_date_edit" class="form-control datepicker1"  autocomplete="off" style="width:49%;float:left;margin-left:10px;" <?php if(!empty($targets[0]['end_date'])){?>value="<?php echo date('d-m-Y',strtotime($targets[0]['end_date']));?>" <?php }?>  placeholder="End Date" minDate="<?php echo date('d-m-Y',strtotime($targets[0]['start_date']));?>">
			</div>
		</div>
		<div class="form-group" style="background:lightgray;margin-top:60px;">
		...
		<div id="sel_deal1">
			<div id="all_int1">
				...
				?>	
						<div class="form-group half_width" ><label><b><?php echo _l('interval'); if($targets[0]['interval'] == 'Monthly'){ echo ' ('.$months[$i].' )';}?></b></label>
							<div>
