<div class="panel_s">
	<div class="panel-body">
		...
		<div class="col-md-12 row">
			...
			<div class="modal fade" id="projects_list_column_orderModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
			  <div class="modal-dialog" role="document">
				<div class="modal-content">
					<?php echo form_open_multipart(admin_url('settings/target_activity_list_column'),array('id'=>'projects_list_column')); ?>
			...
			?>  
			  <ul id="sortable">
			  <?php $targets_list_column_order = (array)json_decode(get_option('target_activity_list_column')); $i = 0;
			  $req_table = array();
			  //print_r($targets_list_column_order);exit; ?>
			  <?php foreach($targets_list_column_order as $ckey=>$cval){
					if($ckey!='pipeline_stage'){
				  ?>
    ...
    render_datatable($table_data,'target-activity');  ?>
			<?php /*<table class="table dt-table scroll-responsive table-project-files" data-order-col="7" data-order-type="desc">
			  <thead>
				<tr>
		...
		<div class="modal-dialog" style="width:50%">
			<div class="modal-content">
				<div class="modal-header bg-blue">
					...
					<h4 class="modal-title"><?php echo _l('add_goal_activity');?></h4>
				</div>
				<div class="col-md-12 bg-white" style="border-radius:6px;">
					...
					<div class="col-md-5" style="margin-top:10px;">
						...
						<div class="tabs active1" id="tab01" style="border-radius:10px;">
							<h6 class="text-muted"><span class="cur_deal"><i class="fa fa-file"></i></span><?php echo _l('activity');?><div class="pull-right dol_sym"><i class="fa fa-angle-right" style="font-size:40px;"></div></i></h6>
						</div>
						...
						<fieldset id="tab011" class="show">
							<form action="" method="post" id="compose_email" enctype='multipart/form-data' >
								...
								<div class="modal-body">
									...
									<div class="form-group">
										<div class="full_cont_div req_class" onclick="show_div('cur_div2','completed')" id="cur_div2">
											<div class="first_cont_div"><?php echo _l('completed');?></div>
											<div class="second_cont_div req_class" id="cur_div21">Based on the number of activities marked as done</div>
										</div>
									</div>
								</div>
							</form>
	...
	<div class="modal-wrapper">
		<div class="modal-dialog">
			<div class="modal-content">
				<div class="modal-header bg-blue">
					<button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
					<h4 class="modal-title"><?php echo _l('edit_goal_activity');?> <span id="goal_txt"></span></h4>
				</div>
	...
	<div class="modal-wrapper">
		<div class="modal-dialog">
			<div class="modal-content">
				<div class="modal-header bg-blue">
					<button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
					<h4 class="modal-title"><?php echo _l('add_goal_activity');?> <span id="goal_txt"></span></h4>
				</div>
				<form action="<?php echo admin_url('target/activity'); ?>" method="post" id="add_deal">
					<input type="hidden" name="<?php echo $this->security->get_csrf_token_name();?>" value="<?php echo $this->security->get_csrf_hash();?>">
					<div class="modal-body">
						<div class="form-group">
						...
						<div class="form-group" id="pipeselect">
							...
							<select class="form-control selectpicker" data-live-search="true" name="select_deal[]" required  multiple data-none-selected-text="<?php echo _l('Select Pipeline'); ?>" onchange="sel_deal()" id="select_deal" data-actions-box='true'>
								<?php if(!empty($deals)){
						...
						<div class="form-group">
							...
							<div>
								<input type="text" name="start_date" class="form-control datepicker1" style="width:49%;float:left" required  autocomplete="off" placeholder="Start Date" onchange="check_end_date()" id="start_date1">
								<input type="hidden" name="goal_val" class="form-control" id="goal_val" >
								<input type="text" name="end_date" class="form-control datepicker1" style="width:49%;float:left;margin-left:10px;" autocomplete="off"  placeholder="End Date" id="end_date1">
							</div>
						</div>
						...
						<div id="sel_deal">
							<div id="all_int">
								<div class="form-group half_width" ><label><b><?php echo _l('interval');?></b></label>
									<div>
										<input type="text" name="intervals[]" class="form-control " readonly value="Weekly">
									</div>
								</div>
...
function sel_deal(){
	...
	/* for(var i =0; i<d;i++){
		 alert(b[i]);
		 if(rad_val == 'Count'){
			req_val = req_val + '<div class="form-group half_width" ><label ><b>INTERVAL</b></label><div><input type="text" name="interval[]" class="form-control " readonly value="'+int_val+'"></div></div><div class="form-group half_width mar_10" ><label class="ch_Count ch_value1"><b>COUNT</b></label><label class="ch_Value ch_value1" style="display:none"><b>VALUE (INR)</b></label><div><input type="number" name="start_date" class="form-control " ></div></div>';
		 }else{
			 req_val = req_val + '<div class="form-group half_width" ><label ><b>INTERVAL</b></label><div><input type="text" name="interval[]" class="form-control " readonly value="'+int_val+'"></div></div><div class="form-group half_width mar_10" ><label class="ch_Count ch_value1" style="display:none"><b>COUNT</b></label><label class="ch_Value ch_value1" ><b>VALUE (INR)</b></label><div><input type="number" name="start_date" class="form-control " ></div></div>';
		 }
	 }
	 $('#sel_deal').html(req_val);*/
}
...
function check_edit(a){
	...
	var data = {target_id:a};
	 var ajaxRequest = $.ajax({
		type: 'POST',
		url: admin_url + 'target/check_edit_activity',
		data: data,
		dataType: '',
		success: function(msg) {
