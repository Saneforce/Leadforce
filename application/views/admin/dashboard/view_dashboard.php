					...
					<div class="col-md-12">
						<div class="panel_s">
							<div class="panel-body">
								...
								<?php
									$table_data = array('Id',_l('dashboard'),_l('create_date'),_l('update_date'));
									render_datatable($table_data,'view_dashboard');  
								?>
							</div>
						</div>
					</div>
...
<div class="modal fade" id="folder_edit_modal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" data-keyboard="false" data-backdrop="static">
	<div class="modal-dialog" role="document">
		...
		<div id="overlay_deal" style="display: none;"><div class="spinner"></div></div>
			<?php echo form_open(admin_url('dashboard/edit_dashboard'),array('id'=>'folder_edit')); ?>
...
$(function(){
    ...
    initDataTable('.table-view_dashboard', admin_url+'dashboard/view_dashboard', notSortableAndSearchableItemColumns, notSortableAndSearchableItemColumns,'undefined',[0,'asc']);
});
		...
		if ( name_val.match(/^[a-zA-Z0-9]+/) && name_val!='' ) {
			$.ajax({
				...
				success: function (data) {
					...
					alert_float('success', 'Dashboard Updated Successfully');
  ...
  function edit_name(a){
	  ...
	  $('#cur_report').val(a);
		var ajaxRequest = $.ajax({
			type: 'POST',
			url: admin_url + 'reports/edit_dashboard',
			data: data,
			dataType: '',
