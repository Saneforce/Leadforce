...
if ($this->ci->input->post('projects_related')) {
    array_push($filter, 'OR rel_type="project"');
}
...
foreach ($rResult as $aRow) {
    ...
    } elseif ($aRow['rel_type'] == 'project') {
        $rel_data = get_relation_data($aRow['rel_type'],$aRow['rel_id']);
        $rel_val = get_relation_values($rel_data,$aRow['rel_type']);
        $toOutput = '<a href="' . admin_url('projects/view/' . $rel_val['id']) . '" target="_blank" data-toggle="tooltip" data-title="' . _l('project') . '">' . $rel_val['name'] . '</a>';
    }
