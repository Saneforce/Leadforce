...
array_push($join, 'LEFT JOIN ' . db_prefix() . 'clients ON ' . db_prefix() . 'clients.userid = ' . $client_id);
...
if ($client_id != '') {
    $d = "AND (  ".db_prefix() . "contacts.userid='$client_id'  or FIND_IN_SET('$client_id',".db_prefix() . "contacts.userids) ) AND " . db_prefix() . "contacts.deleted_status=0 ";
    $where=[$d];
}
...
$result = data_tables_init($aColumns, $sIndexColumn, $sTable, $join, $where, [db_prefix() . 'contacts.id as id', 'tblcontacts.userid as userid', 'is_primary', 'tblclients.addedfrom']);
...
foreach ($rResult as $aRow) {
    ...
    $outputActive = '<div class="onoffswitch">
                <input '.$diabled.' type="checkbox"' . (total_rows(db_prefix() . 'clients', 'registration_confirmed=0 AND userid=' . $aRow['userid']) > 0 ? ' disabled' : '') . ' data-switch-url="' . admin_url() . 'clients/change_contact_status" name="onoffswitch" class="onoffswitch-checkbox" id="c_' . $aRow['id'] . '" data-id="' . $aRow['id'] . '"' . ($aRow['active'] == 1 ? ' checked': '') . '>
