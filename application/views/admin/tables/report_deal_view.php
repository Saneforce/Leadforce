...
foreach ($rResult as $aRow) {
    ...
    for ($i = 0; $i < count($aColumns); $i++) {
		$cur_link = "'".$aRow['id']."'";
		$_data = '<a href="'.admin_url('reports/edit_deal_report/'.$aRow['id']).'"> <i class="fa fa-area-chart" style="font-size:15px"></i>   '.$aRow['report_name'].'</a><div class="row-options"><a href="javascript:void(0);" class="" data-toggle="modal" data-target="#public_add_modal" onclick="load_public('.$cur_link.')">Public Link</a>&nbsp;&nbsp;&nbsp;<a href="' . admin_url('reports/delete_report/' . $aRow['id']) . '" class="text-danger _delete">' . _l('delete') . '</a></div>';
		$row[]              = $aRow['id'];
        $row[]              = $_data;
